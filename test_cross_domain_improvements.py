...
def test_confidence_scoring():
    ...
    for query in confidence_test_queries:
        ...
        if analysis.detected_boundaries:
            boundary = analysis.detected_boundaries[0]
            print(f"Confidence: {boundary.confidence:.3f}")
            print(f"Domains: {boundary.primary_domain.value} + {[d.value for d in boundary.secondary_domains]}")
...
def test_learning_system():
    ...
    for query in learning_queries:
        ...
        if analysis.agent_suggestions:
            ...
            coordinator.record_selection_feedback(
                query, best_agent, confidence, 
                user_feedback=True, task_success=True
            )
    ...
    print(f"  Recent cross-domain usage rate: {selector_insights.get('recent_cross_domain_usage_rate', 0.0):.3f}")
