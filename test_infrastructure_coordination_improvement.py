...
def generate_problematic_test_scenarios() -> List[Tuple[str, str, str, str]]:
    ...
    return [
        ...
        ("Docker container testing framework", 
         "test-specialist", "high", "Testing framework should prioritize testing domain"),
        
        ("Kubernetes performance optimization analysis", 
         "performance-optimizer", "high", "Performance optimization should prioritize performance domain"),
         
        ("Container orchestration performance monitoring", 
         "infrastructure-engineer", "medium", "Primary focus is infrastructure orchestration"),
        
        ("Docker deployment with CI/CD pipeline integration", 
         "infrastructure-engineer", "medium", "Docker deployment is primarily infrastructure"),
        
        ("Kubernetes cluster autoscaling configuration", 
         "infrastructure-engineer", "high", "K8s autoscaling is core infrastructure management"),
        
        ("Infrastructure monitoring dashboard setup", 
         "infrastructure-engineer", "high", "Infrastructure monitoring setup requires infrastructure expertise"),
        
        ("Container security scanning automation", 
         "security-enforcer", "high", "Security scanning should prioritize security domain"),
        
        ("DevOps pipeline optimization for microservices", 
         "infrastructure-engineer", "medium", "DevOps pipeline is infrastructure orchestration"),
        
        ("Service mesh traffic routing configuration", 
         "infrastructure-engineer", "high", "Service mesh configuration is infrastructure networking"),
        
        ("Docker image build optimization", 
         "infrastructure-engineer", "medium", "Docker operations are infrastructure domain"),
        
        # Cross-domain scenarios that need proper disambiguation
        ("Terraform infrastructure testing automation", 
         "infrastructure-engineer", "high", "Primary domain is infrastructure (Terraform)"),
        
        ("Container performance profiling and optimization", 
         "performance-optimizer", "medium", "Primary focus is performance optimization"),
        
        ("Kubernetes security policy enforcement", 
         "security-enforcer", "high", "Primary focus is security policy"),
        
        ("Infrastructure deployment pipeline testing", 
         "infrastructure-engineer", "medium", "Primary domain is infrastructure deployment"),
        
        ("Docker container log aggregation system", 
         "infrastructure-engineer", "medium", "Container logging is infrastructure concern"),
        
        # Complex scenarios that require coordination
        ("Multi-cloud Kubernetes cluster deployment", 
         "infrastructure-engineer", "high", "Multi-cloud K8s is advanced infrastructure orchestration"),
        
        ("Container orchestration with service discovery", 
         "infrastructure-engineer", "high", "Container orchestration is core infrastructure"),
        
        ("Infrastructure as code validation and testing", 
         "infrastructure-engineer", "medium", "IaC is primarily infrastructure domain"),
        
        ("Microservices architecture deployment automation", 
         "infrastructure-engineer", "high", "Microservices deployment is infrastructure orchestration"),
        
        ("Container registry management and automation", 
         "infrastructure-engineer", "medium", "Container registry is infrastructure management"),
    ]
...
def run_targeted_accuracy_test() -> Dict:
    ...
    for query, expected_agent, priority, reasoning in test_scenarios:
        ...
        print(f"{status} {priority_icon} {query[:45]}... → {result.agent_name} ({result.confidence_score:.2f})")
...
def analyze_failure_patterns(results: Dict) -> Dict:
    ...
    for failure in failures:
        ...
        if expected == 'infrastructure-engineer' and selected != 'infrastructure-engineer':
            failure_patterns['infrastructure_to_other'].append(failure)
        elif expected != 'infrastructure-engineer' and selected == 'infrastructure-engineer':
    ...
    for pattern_name, pattern_failures in failure_patterns.items():
        if pattern_failures:
            ...
            for failure in pattern_failures[:2]:  # Show first 2 examples
                print(f"     • {failure['query'][:40]}... → {failure['selected']} (expected: {failure['expected']})")
    
    return {
        'failure_count': len(failures),
        'patterns': failure_patterns,
        'analysis': {
            'most_common_pattern': max(failure_patterns.items(), key=lambda x: len(x[1]))[0] if failures else None,
            'avg_failed_confidence': sum(f['confidence'] for f in failures) / len(failures) if failures else 0
        }
    }

def simulate_learning_improvement(initial_results: Dict, learning_cycles: int = 5) -> Dict:
    ...
    if not failed_scenarios:
        print("🎉 No failed scenarios to learn from - system already performing optimally!")
    ...
    for scenario in failed_scenarios:
        ...
        print(f"   {status} {scenario['query'][:40]}... → {result.agent_name} ({result.confidence_score:.2f})")
    ...
    print(f"   New Overall Accuracy: {new_accuracy:.1f}% (was {initial_results['overall_accuracy']:.1f}%)")
...
def generate_coordination_improvement_report(test_results: Dict, learning_results: Dict, failure_analysis: Dict) -> Dict:
    ...
    final_accuracy = learning_results.get('new_accuracy', test_results['overall_accuracy'])
    ...
    report = {
        ...
        'success_criteria': {
            ...
            'learning_system_effective': learning_results.get('improvement_rate', 0) > 0,
            'high_priority_scenarios_accurate': test_results['high_priority_accuracy'] >= 80.0,
            'overall_success': accuracy_target_met and performance_target_met
        },
        'recommendations': generate_recommendations(test_results, learning_results, failure_analysis)
    }
    
    return report

def generate_recommendations(test_results: Dict, learning_results: Dict, failure_analysis: Dict) -> List[str]:
    ...
    if final_accuracy < 75.0:
        recommendations.append(f"Accuracy ({final_accuracy:.1f}%) below 75% target - consider additional pattern refinement")
    
    if improvement_rate > 0:
        recommendations.append(f"Learning system effective ({improvement_rate:.1f}% improvement) - continue training cycles")
    
    if test_results['avg_response_time_ms'] > 100.0:
        recommendations.append("Response times good but could be optimized further with caching")
...
def save_improvement_report(report: Dict, filename: str = "infrastructure_coordination_improvement_report.json") -> Path:
...
def print_improvement_summary(report: Dict):
    ...
    if success['overall_success']:
        ...
        print("   System ready for production use with excellent accuracy and performance.")
    else:
