...
def test_learning_enhancement():
    ...
    for i, (query, expected_agent) in enumerate(test_patterns, 1):
        ...
        print(f'   {i}. "{query[:40]}..."')
        print(
            f"      Selected: {result.agent_name} (confidence: {result.confidence_score:.2f})"
        )
    ...
    correct_baseline = sum(
        1
        for (_, actual, _), (_, expected) in zip(baseline_results, test_patterns)
        if actual == expected
    )
    baseline_accuracy = correct_baseline / len(test_patterns)

    print(
        f"📈 Baseline Accuracy: {baseline_accuracy:.1%} ({correct_baseline}/{len(test_patterns)})"
    )
    ...
    for i, (query, expected_agent) in enumerate(test_patterns, 1):
        ...
        baseline_agent, baseline_conf = (
            baseline_results[i - 1][1],
            baseline_results[i - 1][2],
        )

        print(f'   {i}. "{query[:40]}..."')
        print(f"      Before: {baseline_agent} (confidence: {baseline_conf:.2f})")
        print(
            f"      After:  {result.agent_name} (confidence: {result.confidence_score:.2f})"
        )

        # Show improvement
        if result.confidence_score > baseline_conf:
            improvement = (
                (result.confidence_score - baseline_conf) / baseline_conf
            ) * 100
            print(f"      🚀 Improved confidence by {improvement:.1f}%")
        elif (
            result.agent_name != baseline_agent and result.agent_name == expected_agent
        ):
    ...
    correct_improved = sum(
        1
        for (_, actual, _), (_, expected) in zip(improved_results, test_patterns)
        if actual == expected
    )
    ...
    if insights:
        ...
        print(
            f'   Context Patterns Learned: {insights.get("context_patterns_learned", 0)}'
        )
...
if __name__ == "__main__":
    try:
        ...
        print("=" * 70)
        print(
            f'Learning Enhancement: {"PASSED" if improved >= baseline else "NEEDS IMPROVEMENT"}'
        )
