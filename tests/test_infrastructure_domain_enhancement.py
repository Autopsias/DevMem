...
class TestInfrastructureDomainEnhancement:
    ...
    def infrastructure_edge_cases(self) -> List[Tuple[str, float]]:
        ...
        return [
            ...
            (
                "Container testing pipeline configuration",
                0.70,
    ...
    def test_infrastructure_agent_selection_accuracy(
        self, selector, infrastructure_test_cases
    ):
        ...
        for query, expected_confidence in infrastructure_test_cases:
            ...
            ), f"Low confidence ({result.confidence_score:.2f}) for: {query}"

            # Should ideally meet expected confidence
            if (
                result.confidence_score < expected_confidence * 0.8
            ):  # Allow 20% tolerance
                print(
                    f"Below expected confidence for '{query}': {result.confidence_score:.2f} < {expected_confidence:.2f}"
                )
        ...
        assert (
            accuracy >= 0.90
        ), f"Infrastructure domain accuracy {accuracy:.1%} below 90% target"
        assert (
            avg_confidence >= 0.75
        ), f"Average confidence {avg_confidence:.2f} too low"
        assert (
            avg_processing_time < 5.0
        ), f"Processing time {avg_processing_time:.1f}ms too slow"
    ...
    def test_infrastructure_pattern_matching(self, selector):
        ...
        for query, expected_patterns in pattern_tests:
            ...
            # Should select infrastructure-engineer
            assert (
                result.agent_name == "infrastructure-engineer"
            ), f"Wrong agent for '{query}': {result.agent_name}"

            # Should have reasonable confidence
            assert (
                result.confidence_score >= 0.7
            ), f"Low confidence for '{query}': {result.confidence_score:.2f}"
            ...
            assert (
                len(overlap) >= 1
            ), f"No pattern overlap for '{query}': {matched_keywords} vs {expected_keywords}"
    ...
    def test_infrastructure_keyword_extraction(self, selector):
        ...
        keyword_tests = [
            ...
            ),  # devops -> infrastructure, pipeline -> deployment
        ...
        for query, expected_keywords in keyword_tests:
            ...
            for expected_keyword in expected_keywords:
                ...
                ), f"Missing keyword '{expected_keyword}' in '{query}'"
    ...
    def test_infrastructure_edge_cases(self, selector, infrastructure_edge_cases):
        ...
        for query, min_confidence in infrastructure_edge_cases:
            ...
            assert (
                result.confidence_score >= min_confidence * 0.8
            ), f"Very low confidence for edge case '{query}': {result.confidence_score:.2f}"
            ...
            print(
                f"Edge case: '{query}' -> {result.agent_name} ({result.confidence_score:.2f})"
            )
        ...
        assert (
            edge_accuracy >= 0.70
        ), f"Infrastructure edge case accuracy {edge_accuracy:.1%} too low"
    ...
    def test_infrastructure_vs_other_domains(self, selector):
        ...
        disambiguation_tests = [
            ...
            (
                ...
                "test-specialist",
            ),  # Testing context stronger
            (
                "Container deployment pipeline testing",
                "infrastructure-engineer",
            ),  # Infrastructure context stronger
            # Infrastructure vs Security
            (
                "Container security vulnerability scanning",
                "security-enforcer",
            ),  # Security context stronger
            (
                "Infrastructure security hardening deployment",
                "infrastructure-engineer",
            ),  # Infrastructure context stronger
        ...
        for query, expected_agent in disambiguation_tests:
            ...
            if result.agent_name == expected_agent:
                ...
                print(
                    f"Disambiguation issue: '{query}' -> {result.agent_name} (expected: {expected_agent})"
                )
        ...
        assert (
            disambiguation_accuracy >= 0.80
        ), f"Domain disambiguation accuracy {disambiguation_accuracy:.1%} too low"

    def test_infrastructure_performance_requirements(
        self, selector, infrastructure_test_cases
    ):
        ...
        test_queries = [
            query for query, _ in infrastructure_test_cases
        ] * 5  # 5x repetition
        ...
        ), f"Average processing time {avg_processing_time:.2f}ms too slow"
        assert (
            max_processing_time < 10.0
        ), f"Maximum processing time {max_processing_time:.2f}ms too slow"
    ...
    def test_infrastructure_comprehensive_validation(self, selector):
        ...
        for scenario in comprehensive_scenarios:
            ...
            print(
                f"Comprehensive: '{scenario}' -> {result.agent_name} ({result.confidence_score:.2f})"
            )
        ...
        assert (
            comprehensive_accuracy >= 0.90
        ), f"Comprehensive accuracy {comprehensive_accuracy:.1%} below target"
        assert (
            high_confidence_rate >= 0.75
        ), f"High confidence rate {high_confidence_rate:.1%} too low"
