...
class IntegratedValidationFramework:
        ...
        for suite_name, validator in validation_suites:
            ...
            try:
                ...
                suite = ValidationSuite(
                    name=suite_name,
                    results=suite_result if isinstance(suite_result, list) else 
                           [ValidationResult(suite_name, suite_result)],
                    overall_passed=all(r.passed for r in suite_result) if isinstance(suite_result, list) else suite_result,
                ...
                if suite.overall_passed:
                    print(f"✅ {suite_name}: PASSED ({suite.passed_count}/{suite.total_count})")
                else:
                    print(f"❌ {suite_name}: FAILED ({suite.passed_count}/{suite.total_count})")
    ...
    def _validate_s63_implementation(self) -> List[ValidationResult]:
        ...
        coordination_file = test_root / "agent_coordination" / "test_coordination_patterns.py"
        coordination_valid = False
        if coordination_file.exists():
            content = coordination_file.read_text()
            coordination_valid = all(pattern in content for pattern in [
        ...
        if performance_file.exists():
            ...
            performance_valid = all(pattern in content for pattern in [
        ...
        if coordination_file.exists():
            content = coordination_file.read_text()
            sequential_parallel_valid = "sequential" in content.lower() and "parallel" in content.lower()
        ...
        integration_file = test_root / "agent_coordination" / "test_integration_scenarios.py"
        integration_valid = False
        if integration_file.exists():
            content = integration_file.read_text()
            integration_valid = all(pattern in content for pattern in [
    ...
    def _validate_native_config(self) -> List[ValidationResult]:
        ...
        try:
            ...
            if 'agents' in config:
                ...
                agents_valid = all(key in agents_config for key in [
                    'version', 'framework_config', 'performance_targets'
                ])
                results.append(ValidationResult(
        ...
        for test_class, description in test_suites:
            try:
                ...
                results.append(ValidationResult(
                    description,
                    test_passed,
                    score=passed_count / (passed_count + failed_count) if (passed_count + failed_count) > 0 else 0,
    ...
    def _validate_agent_framework_structure(self) -> List[ValidationResult]:
        ...
        results.append(ValidationResult(
            ...
            found_secondary_required >= len(required_secondary_agents) * 0.8,  # 80% should exist
    ...
    def _generate_final_report(self, overall_success: bool) -> None:
        ...
        for suite in self.test_results:
            ...
            print(f"{status} {suite.name} ({suite.passed_count}/{suite.total_count}) - {suite.total_time_ms:.1f}ms")
        ...
        if self.test_results:
            avg_suite_time = statistics.mean(suite.total_time_ms for suite in self.test_results)
        ...
        if overall_success:
            ...
            failed_suites = [suite.name for suite in self.test_results if not suite.overall_passed]
...
class TestIntegratedValidationFramework:
    ...
    def test_story_completion_validation(self, validation_framework):
        ...
        assert passed_count > 0, "At least some story completion validations should pass"
    ...
    def test_agent_framework_validation(self, validation_framework):
        ...
        dir_structure_result = next((r for r in results if "Directory Structure" in r.name), None)
...
def main():
    ...
    parser.add_argument(
        "--suite", 
        choices=["story", "s63", "agent-selection", "infrastructure", "config", "learning", "all"],
        default="all",
