    ...
    def __init__(self):
        self.agent_patterns = {
            "test-specialist": [
                ...
                r"coverage.*gap",
                r"test.*suite",
                r"pytest.*test.*failing",
                r"async.*mock",
                r"testing.*framework",
                r"test.*framework.*config",
            ],
            "infrastructure-engineer": [
                ...
                r"deployment.*issue",
                r"container.*deployment",
                r"deployment.*pipeline",
                r"orchestration.*issues",
            ],
            "security-enforcer": [
                r"security.*pattern",
...
class TestAgentPatternMatching:
    ...
    def test_queries(self):
        return [
            ...
            TestQuery(
                ...
                variations=[
                    "deploy container pipeline",
                    "container deploy optimization",
                ],
            ),
        ]
    ...
    def test_current_matcher_accuracy(self, current_matcher, test_queries):
        ...
        ), f"Current matcher too slow: {avg_processing_time:.2f}ms"
    ...
    def test_enhanced_matcher_accuracy(self, enhanced_matcher, test_queries):
        ...
        for test_query in test_queries:
            ...
            assert (
                min_conf <= result.confidence_score <= max_conf
        ...
        ), f"Enhanced matcher too slow: {avg_processing_time:.2f}ms"
        assert (
            avg_confidence >= 0.65
        ), f"Average confidence too low: {avg_confidence:.2f}"

    def test_performance_comparison(
        self, current_matcher, enhanced_matcher, test_queries
    ):
        ...
        print(
            f"Current: {current_accuracy_pct:.2%} accuracy, {current_avg_time:.2f}ms avg time"
        )
        print(
            f"Enhanced: {enhanced_accuracy_pct:.2%} accuracy, {enhanced_avg_time:.2f}ms avg time"
        )

        accuracy_improvement = (
            enhanced_accuracy_pct - current_accuracy_pct
        ...
        assert (
            enhanced_accuracy_pct > current_accuracy_pct
        ), "Enhanced matcher should be more accurate"
        # Allow enhanced matcher to be slightly slower due to more sophisticated processing
        assert (
            enhanced_avg_time < current_avg_time * 2.0
        ), "Enhanced matcher should not be significantly slower"
    ...
    def test_high_load_performance(self, enhanced_matcher):
        ...
        assert (
            avg_time_per_query < 2.0
        ), f"High load performance too slow: {avg_time_per_query:.2f}ms"
