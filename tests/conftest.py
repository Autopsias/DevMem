...
def test_config():
    ...
    return {
        ...
        "test_results_dir": Path("/Users/ricardocarvalho/DeveloperFolder/DevMem/tests/results"),
    ...
    class AgentValidator:
        ...
        def validate_agent_content(self, agent_name: str) -> Dict[str, Any]:
            ...
            return {
                ...
                "has_tools": any(tool in content for tool in ["Read", "Edit", "Bash", "Task"]),
...
def coordination_tester():
    ...
    class CoordinationTester:
        ...
        def test_parallel_coordination(self, agents: List[str]) -> Dict[str, Any]:
            ...
            results["total_time"] = time.time() - start_time
            results["parallel_efficiency"] = results["total_time"] < (0.02 * len(agents))
            
            return results
        
        def validate_coordination_results(self, results: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if results.get("coordination_type") == "sequential":
                if not results.get("context_preserved", False):
                    validation["valid"] = False
                    validation["issues"].append("Context not preserved in sequential coordination")
    ...
    class TestReporter:
        ...
        def generate_performance_report(self, performance_results: Dict[str, Any]) -> str:
            ...
            for test_name, metrics in performance_results.items():
                ...
                if exec_time > 1000:
                    report.append(f"  ⚠️  Action: Optimize {test_name} - execution time exceeds 1s")
                if memory_delta > 100:
                    report.append(f"  ⚠️  Action: Review {test_name} memory usage - high memory consumption")
                if cpu_usage > 80:
                    report.append(f"  ⚠️  Action: Investigate {test_name} CPU usage - high CPU load")
...
def pytest_configure(config):
    ...
    # Add custom markers
    config.addinivalue_line("markers", "coordination: mark test as coordination pattern test")
    config.addinivalue_line("markers", "performance: mark test as performance benchmark test")
...
def coordination_scenario(request):
    ...
    scenarios = {
        ...
        "parallel_with_synthesis": {
            "type": "parallel",
            "agents": ["test-specialist", "infrastructure-engineer", "synthesis-coordinator"],
            "expected_time": 0.8
        },
        "hybrid_coordination": {
            "type": "hybrid",
