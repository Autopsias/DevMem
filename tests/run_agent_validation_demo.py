...
def run_basic_demo():
    ...
    print("   - Domain-specific patterns: 5 per domain (testing, infrastructure, security, performance, code_quality)")
    print("   - Edge case patterns: 8 patterns")
    patterns = framework.generate_comprehensive_test_suite(patterns_per_domain=5, edge_cases=8)
...
def run_detailed_analysis():
    ...
    patterns = framework.generate_comprehensive_test_suite(patterns_per_domain=15, edge_cases=10)
    ...
    saved_path = framework.save_validation_report(report, "detailed_validation_report.json")
...
def run_performance_benchmark():
    ...
    print(f"{'Scale':<8} {'Time (s)':<10} {'Patterns/s':<12} {'Accuracy':<10} {'Recommendations'}")
    print("-" * 60)
    
    for scale in test_scales:
        patterns = framework.generate_comprehensive_test_suite(
            patterns_per_domain=scale//5, 
            edge_cases=max(5, scale//10)
        )
        ...
        print(f"{len(patterns):<8} {execution_time:<10.2f} {patterns_per_second:<12.1f} {report.overall_accuracy:<10.2%} {len(report.improvement_recommendations)}")
...
def main():
    parser = argparse.ArgumentParser(description="Agent Selection Validation Framework Demo")
    parser.add_argument('--mode', choices=['basic', 'detailed', 'performance', 'edge-cases'], 
                       default='basic', help='Demo mode to run')
    parser.add_argument('--save-reports', action='store_true', 
                       help='Save validation reports to files')
