...
class TestEnhancedBoundaryDetection:
    ...
    def test_multi_domain_detection(self, boundary_detector):
        ...
        for query in multi_domain_queries:
            ...
            if boundaries:
                assert (
                    len(boundaries[0].secondary_domains) >= 1
                ), f"Should detect multiple domains for: {query}"
    ...
    def test_complex_boundary_patterns(self, boundary_detector):
        ...
        for query in complex_queries:
            ...
            assert (
                len(boundaries) >= 1
            ), f"Should detect complex boundaries for: {query}"
            if boundaries:
                boundary = boundaries[0]
                assert (
                    boundary.complexity_score >= 1.0
                ), f"Should detect complexity for: {query}"
                assert (
                    len(boundary.overlap_indicators) > 0
                ), f"Should detect overlap indicators for: {query}"
    ...
    def test_boundary_confidence_scoring(self, boundary_detector):
        ...
        for query, expected_min_confidence in confidence_test_cases:
            ...
            if boundaries:
                assert (
                    boundaries[0].confidence >= expected_min_confidence
    ...
    def test_overlap_indicator_detection(self, boundary_detector):
        ...
        overlap_types = ["integration_signal", "parallel_signal", "dependency_signal"]
        ...
        for query, expected_type in zip(overlap_queries, overlap_types):
            ...
            if boundaries:
                ...
                assert any(
                    expected_type in indicator for indicator in overlap_indicators
...
class TestConflictDetectionEngine:
    ...
    def test_security_performance_conflict_detection(
        self, conflict_engine, sample_boundaries
    ):
        ...
        for query in conflict_queries:
            ...
            assert (
                len(security_performance_conflicts) >= 1
    ...
    def test_resource_competition_detection(self, conflict_engine):
        ...
        for query in resource_queries:
            ...
            resource_conflicts = [
                c
                for c in conflicts
                if c.conflict_type == ConflictType.RESOURCE_COMPETITION
            ]

            assert (
                len(resource_conflicts) >= 1
    ...
    def test_resolution_strategy_generation(self, conflict_engine, sample_boundaries):
        ...
        if security_performance_conflicts:
            ...
            assert (
                "hardware acceleration" in strategy_text or "caching" in strategy_text
...
class TestEnhancedCrossDomainCoordinator:
    ...
    def test_comprehensive_analysis(self, coordinator):
        ...
        for query in complex_queries:
            ...
            assert (
                len(analysis.detected_boundaries) >= 1
    ...
    def test_coordination_recommendation_generation(self, coordinator):
        ...
        coordination_test_cases = [
            ...
            (
                "complex multi-domain infrastructure security performance testing",
                "meta-coordination",
            ),
        ]
    ...
    def test_integration_complexity_calculation(self, coordinator):
        ...
        complexity_test_cases = [
            ...
            (
                "multi-cloud infrastructure security performance testing documentation",
                0.7,
                1.0,
            ),  # High complexity
        ]
...
class TestAgentSelectorIntegration:
    ...
    def test_cross_domain_enhanced_selection(self, selector):
        ...
        for query in cross_domain_queries:
            ...
            assert (
                "cross-domain" in result.reasoning.lower()
                or "coordination" in result.reasoning.lower()
                or "meta" in result.reasoning.lower()
    ...
    def test_fallback_to_original_selection(self, selector):
        ...
        with patch.object(selector, "cross_domain_coordinator", None):
            ...
            assert (
                "pattern-based" in result.reasoning.lower()
                or "multi-domain" in result.reasoning.lower()
                or "matches" in result.reasoning.lower()
            )
...
class TestPerformanceAndScalability:
    ...
    def test_analysis_performance(self, coordinator):
        ...
        for query in test_queries:
            ...
            assert (
                analysis.processing_time_ms < 200
    ...
    def test_memory_usage_control(self, coordinator):
        ...
        assert (
            len(coordinator.analysis_history) <= 500
...
class TestPatternLearningEngine:
    ...
    def test_infrastructure_query_classification(self, learning_engine):
        ...
        for query, expected_type in test_cases:
            ...
            ), f"Query '{query}' should be classified as '{expected_type}', got '{result}'"
    ...
    def test_learning_from_failure(self, learning_engine):
        ...
        learning_engine.learn_from_failure(
            query, selected_agent, expected_agent, reasons
        )
    ...
    def test_learning_statistics(self, learning_engine):
        ...
        # Add some patterns
        learning_engine.learn_from_success(
            "docker setup", "infrastructure-engineer", 0.8
        )
        learning_engine.learn_from_failure(
            "k8s deploy", "test-specialist", "infrastructure-engineer", ["wrong domain"]
        )
...
class TestEnhancedLearningCoordinator:
    ...
    def learning_coordinator(self, tmp_path):
        ...
        hub_path = tmp_path / "coordination-hub.md"
        coordinator.pattern_learning_engine = PatternLearningEngine(
    ...
    def test_infrastructure_learning_integration(self, learning_coordinator):
        ...
        learning_coordinator.record_selection_feedback(
            query, agent, 0.85, user_feedback=True
        )
    ...
    def test_learning_confidence_boost(self, learning_coordinator):
        ...
        performance_suggestions = [
            s for s in analysis.agent_suggestions if s[0] == "performance-optimizer"
        ]
        ...
        for query in infrastructure_queries:
            ...
            if analysis.agent_suggestions:
                learning_coordinator.record_selection_feedback(
                    ...
                    analysis.agent_suggestions[0][1],
                    user_feedback=True,
                )
    ...
    def test_learning_pattern_persistence(self, tmp_path):
        ...
        coordinator1.pattern_learning_engine = PatternLearningEngine(
            coordination_hub_path=str(hub_path)
        )
        ...
        coordinator2.pattern_learning_engine = PatternLearningEngine(
            coordination_hub_path=str(hub_path)
        )
...
class TestLearningPerformanceImprovements:
    ...
    def test_accuracy_improvement_measurement(
        self, baseline_coordinator, learning_coordinator
    ):
        ...
        for query, correct_agent in test_cases:
            # Baseline analysis
            baseline_analysis = baseline_coordinator.analyze_cross_domain_integration(
                query
            )
            if (
                baseline_analysis.agent_suggestions
                and baseline_analysis.agent_suggestions[0][0] == correct_agent
            ):
                baseline_correct += 1

            # Learning analysis
            learning_analysis = learning_coordinator.analyze_cross_domain_integration(
                query
            )
            if (
                learning_analysis.agent_suggestions
                and learning_analysis.agent_suggestions[0][0] == correct_agent
            ):
                learning_correct += 1
