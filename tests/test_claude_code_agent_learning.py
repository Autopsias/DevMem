...
class TestTaskToolIntegration:
    ...
    def test_task_parallel_coordination_patterns(self, enhanced_selector):
        ...
        for query, expected_agents in task_coordination_patterns:
            ...
            has_coordination_reasoning = any(
            ...
            assert (
                has_coordination_reasoning or result.confidence_score >= 0.75
    ...
    def test_task_tool_performance_targets(self, enhanced_selector):
        ...
        ), f"Task coordination too slow: {avg_response_time:.1f}ms"
    ...
    def test_task_coordination_accuracy_benchmarks(self, enhanced_selector):
        ...
        for query, expected_agent, expected_success_rate in coordination_benchmarks:
            ...
            expected_confidence = max(
                0.4, expected_success_rate - 0.4
            )  # More realistic confidence threshold
            assert (
        ...
        assert (
            accuracy >= 0.6
        ), f"Task coordination accuracy {accuracy:.1%} below 60% target"
...
class TestLearningPatternValidation:
    ...
    def test_infrastructure_learning_patterns(self, enhanced_selector):
        ...
        infrastructure_learning_tests = [
            ...
            (
                "container orchestration networking optimization",
            ...
            (
                "performance scaling infrastructure optimization",
        ...
        assert (
            accuracy >= 0.6
        ), f"Infrastructure learning accuracy {accuracy:.1%} below 60% target"
        assert (
            confidence_rate >= 0.5
        ), f"High confidence rate {confidence_rate:.1%} below 50% target"
    ...
    def test_learning_metrics_compliance(self, enhanced_selector):
        ...
        assert (
            expected_metrics["total_successful_patterns"] >= 200
        ), "Pattern count target reasonable"
        assert (
            expected_metrics["learning_rate"] >= 0.8
        ), "Learning rate target achievable"
        assert (
            expected_metrics["active_query_types"] >= 3
        ), "Query type diversity adequate"
        assert (
            1.0 <= expected_metrics["average_pattern_weight"] <= 5.0
        ), "Pattern weights reasonable"

        # Test pattern weight distribution makes sense
        assert (
            expected_metrics["average_pattern_weight"] > 1.0
        ), "Patterns should be weighted above baseline"
    ...
    def test_pattern_learning_integration(self, enhanced_selector):
        ...
        learning_scenarios = [
            ...
            (
                "docker container networking issues troubleshooting",
                "infrastructure-engineer",
            ),
            ("kubernetes scaling performance bottlenecks", "performance-optimizer"),
            (
                "ingress controller networking configuration problems",
                "infrastructure-engineer",
            ),
            # Cross-domain queries that should show learning influence
            (
                "infrastructure security monitoring setup",
                ["infrastructure-engineer", "security-enforcer"],
            ),
            (
                "performance testing coordination automation",
                ["performance-optimizer", "test-specialist"],
            ),
        ]
        ...
        for query, expected_agents in learning_scenarios:
            ...
            if isinstance(expected_agents, list):
                ...
                assert (
                    result.agent_name == expected_agents
...
class TestAgentDirectoryIntegration:
    ...
    def test_agent_directory_loading(self, enhanced_selector):
        ...
        missing_agents = [
            agent for agent in primary_agents if agent not in loaded_agents
        ]
        assert (
            len(missing_agents) <= 2
        ), f"Too many missing agents from .claude/agents/: {missing_agents}"

        # Should have reasonable total agent count (primary + secondary)
        assert (
            len(loaded_agents) >= 15
        ), f"Expected at least 15 agents, got {len(loaded_agents)}"
    ...
    def test_agent_specialization_accuracy(self, enhanced_selector):
        ...
        specialization_tests = [
            ...
            (
                "docker container orchestration networking setup",
                "infrastructure-engineer",
            ),
            ...
            (
                "complex multi-domain infrastructure security performance",
                "meta-coordinator",
            ),
        ...
        assert (
            accuracy >= 0.5
        ), f"Agent specialization accuracy {accuracy:.1%} below 50% target"
...
class TestMemorySystemPerformance:
    ...
    def test_memory_access_performance_targets(self, enhanced_selector):
        ...
        ), f"Average response time {avg_response_time:.1f}ms exceeds 100ms target"

        # Selection latency should be reasonable (coordination-hub.md target: <0.5s)
        assert (
            max_response_time < 500
        ), f"Max response time {max_response_time:.1f}ms exceeds 500ms target"
    ...
    def test_context_preservation_accuracy(self, enhanced_selector):
        ...
        for query, expected_complexity in context_test_scenarios:
            ...
            has_domain_context = any(
                domain in result.reasoning.lower()
                for domain in [
                    "testing",
                    "infrastructure",
                    "security",
...
class TestAgentDelegationCoordination:
    ...
    def test_sequential_delegation_patterns(self, enhanced_selector):
        ...
        for query, expected_agents in sequential_patterns:
            ...
            has_delegation_reasoning = any(
                indicator in reasoning_lower for indicator in delegation_indicators
            )

            # Allow some flexibility in reasoning detection
            assert (
                has_delegation_reasoning or len(result.reasoning) > 40
    ...
    def test_parallel_coordination_patterns(self, enhanced_selector):
        ...
        for query, expected_coordinators in parallel_patterns:
            ...
            assert (
                result.agent_name in expected_coordinators
