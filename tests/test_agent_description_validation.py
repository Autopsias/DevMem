...
class AnthropicGuidelinesValidator:
    ...
    def validate_agent_structure(self, content: str) -> Dict[str, bool]:
        ...
        compliance["has_frontmatter"] = bool(
    ...
    def analyze_description_quality(self, description: str) -> Dict[str, float]:
        ...
        specific_terms = len(
            re.findall(
                r"\b(?:pytest|docker|kubernetes|async|mock|API|documentation|security|infrastructure|performance|testing)\b",
                description,
                re.IGNORECASE,
            )
        )
        generic_terms = len(
            re.findall(
                r"\b(?:help|issue|problem|work|thing|stuff|handle)\b",
                description,
                re.IGNORECASE,
            )
        )
        quality_scores["specificity"] = specific_terms / max(
            specific_terms + generic_terms, 1
        )
...
class AgentDescriptionTestFramework:
    ...
    def __init__(self, agents_dir: Optional[Path] = None):
        ...
        self.agents_dir = (
            agents_dir or Path(__file__).parent.parent / ".claude" / "agents"
        )
    ...
    def validate_trigger_keyword_extraction(
        self, agent_name: str, content: str
    ) -> Dict[str, any]:
        ...
        return {
            ...
            "has_technical_terms": len(
                [kw for kw in extracted_keywords if kw not in explicit_keywords]
            )
            > 0,
    ...
    def test_agent_selection_integration(
        self, agent_name: str, content: str
    ) -> Dict[str, any]:
    ...
    def _identify_validation_errors(
        self, compliance: Dict[str, bool], quality: Dict[str, float]
    ) -> List[str]:
        ...
        if not compliance.get("clear_triggers", False):
            errors.append(
                "Description lacks clear usage triggers (e.g., 'use for', 'perfect for')"
            )

        if (
            quality.get("keyword_density", 0)
            < self.validator.quality_metrics["keyword_density"]
        ):
            errors.append("Low trigger keyword density in description")

        if (
            quality.get("specificity", 0)
            < self.validator.quality_metrics["specificity_score"]
        ):
...
class TestAgentFileLoading:
    ...
    def test_agent_files_exist(self):
        ...
        assert (
            framework.agents_dir.exists()
        ), f"Agents directory not found: {framework.agents_dir}"
    ...
    def test_frontmatter_parsing(self):
        ...
        assert (
            len(frontmatter_failures) == 0
        ), f"Agents with frontmatter issues: {frontmatter_failures}"
...
class TestAnthropicGuidelinesCompliance:
    ...
    def test_description_structure_compliance(self):
        ...
        ), f"Agents failing compliance: {compliance_failures}"
    ...
    def test_description_quality_standards(self):
        ...
        for agent_name, content in agent_contents.items():
            ...
            if (
                quality_scores.get("length_appropriateness", 1) < 0.6
            ):  # Relaxed threshold
        ...
        severe_issues = {
            name: issues for name, issues in quality_issues.items() if len(issues) >= 3
        }
        severe_issue_ratio = len(severe_issues) / len(agent_contents)
        assert (
            severe_issue_ratio <= 0.3
        ), f"Too many agents with severe quality issues ({100*severe_issue_ratio:.1f}%): {list(severe_issues.keys())}"
    ...
    def test_proactive_usage_indicators(self):
        ...
        for agent_name, content in agent_contents.items():
            ...
            has_proactive = bool(
                re.search(
                    r"(?i)use\s+proactively|proactive|automatically|when\s+detecting",
                    content,
                )
            )
            has_triggers = bool(
                re.search(
                    r"(?i)use\s+for|perfect\s+for|when\s+users?|specializes\s+in",
                    content,
                )
            )
        ...
        proactive_ratio = (len(agent_contents) - len(missing_proactive)) / len(
            agent_contents
        )
        assert (
            proactive_ratio >= 0.7
        ), f"Too few agents with proactive indicators: {100*proactive_ratio:.1f}% (need e70%)"
...
class TestTriggerKeywordExtraction:
    ...
    def test_keyword_extraction_accuracy(self):
        ...
        success_ratio = (len(agent_contents) - len(extraction_failures)) / len(
            agent_contents
        )
        assert (
            success_ratio >= 0.8
        ), f"Keyword extraction success too low: {100*success_ratio:.1f}% (need e80%)"
    ...
    def test_explicit_keyword_coverage(self):
        ...
        assert (
            explicit_ratio >= 0.6
        ), f"Too few agents with explicit keywords: {100*explicit_ratio:.1f}% (need e60%)"
        assert (
            total_explicit_keywords >= 10
        ), f"Total explicit keywords too low: {total_explicit_keywords} (need e10)"
    ...
    def test_technical_term_recognition(self):
        ...
        assert (
            technical_ratio >= 0.4
        ), f"Too few agents with technical terms: {100*technical_ratio:.1f}% (need ≥40%)"
...
class TestAgentSelectionIntegration:
    ...
    @pytest.mark.skipif(
        not AGENT_SELECTOR_AVAILABLE, reason="Agent selector not available"
    )
    def test_agent_selection_integration(self):
        ...
        if tested_agents > 0:
            ...
            assert (
                avg_accuracy >= 0.2
            ), f"Agent selection integration accuracy too low: {100*avg_accuracy:.1f}% (need ≥20%)"
    ...
    def test_core_agent_recognition(self):
        ...
        assert (
            accuracy >= 0.5
        ), f"Core agent recognition too low: {100*accuracy:.1f}% (need e50%)"
...
class TestSuccessPatternRecording:
    ...
    def test_success_pattern_recording(self):
        ...
        test_patterns = [
            (
                ...
                {"keywords": ["pytest", "async"], "accuracy": 0.9},
            ),
            (
                "infrastructure-engineer",
                "description_quality",
                {"specificity": 0.8, "clarity": 0.9},
            ),
            (
                "security-enforcer",
                "compliance_check",
                {"structure": True, "guidelines": True},
            ),
        ]
    ...
    def test_pattern_learning_integration(self):
                ...
                if len(analysis.validation_errors) <= 3:  # Allow some validation issues
                    framework.record_success_pattern(
                        ...
                        "validation_success",
                        {
                            ...
                            "quality_score": sum(analysis.content_quality.values())
                            / len(analysis.content_quality),
                            "compliance_score": sum(
                                analysis.anthropic_compliance.values()
                            )
                            / len(analysis.anthropic_compliance),
                        },
                    )
        ...
        assert (
            success_ratio >= 0.3
        ), f"Success pattern recording ratio too low: {100*success_ratio:.1f}% (need ≥30%)"
...
class TestIntegrationScenarios:
    ...
    def test_comprehensive_validation_workflow(self):
        ...
        for agent_name, content in agent_contents.items():
            try:
                ...
                if framework.agent_selector:
                    integration_result = framework.test_agent_selection_integration(
                        agent_name, content
                    )
        ...
        assert (
            workflow_results["analyzed_agents"]
            >= workflow_results["loaded_agents"] * 0.9
        ), "Analysis coverage too low"
        assert (
            workflow_results["compliant_agents"]
            >= workflow_results["analyzed_agents"] * 0.5
        ), "Compliance ratio too low"
        assert (
            workflow_results["agents_with_keywords"]
            >= workflow_results["analyzed_agents"] * 0.8
        ), "Keyword extraction ratio too low"
    ...
    def test_agent_file_consistency(self):
        ...
        consistency_metrics["average_description_length"] = total_desc_length / max(
            total_agents, 1
        )

        # Validate consistency
        assert (
            consistency_metrics["agents_with_frontmatter"] >= total_agents * 0.95
        ), "Too many agents missing frontmatter"
        assert (
            consistency_metrics["agents_with_tools"] >= total_agents * 0.9
        ), "Too many agents missing tools"
        assert (
            consistency_metrics["agents_with_descriptions"] >= total_agents * 0.95
        ), "Too many agents missing descriptions"
        assert (
            consistency_metrics["average_description_length"] >= 50
        ), "Average description too short"
        assert (
            consistency_metrics["total_trigger_keywords"] >= total_agents * 2
        ), "Too few trigger keywords overall"
