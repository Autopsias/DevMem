...
class DomainSpecificPatternGenerator:
    ...
    def __init__(self):
        self.domain_templates = {
            "testing": [
                ("Test failures in pytest with asyncio patterns", "test-specialist", ["testing"], "sequential"),
                ("Mock configuration issues causing test isolation problems", "test-specialist", ["testing"], "sequential"),
                ("Coverage gaps in integration testing suite", "test-specialist", ["testing"], "sequential"),
            ],
            "infrastructure": [
            ...
            "performance": [
                ...
                ("Memory usage spiking causing OOM errors", "performance-optimizer", ["performance"], "sequential"),
            ],
            "code_quality": [
        ...
        self.pattern_type_modifiers = {
            ...
            "contextual": ["Following the previous analysis, ", "Building on our earlier work, "],
    ...
    def generate_patterns(self, count: int, domain: str = None) -> List[TestPattern]:
        ...
        for i in range(count):
            ...
            input_text, expected_agent, expected_domains, expected_coordination = template
            
            # Add complexity variation
            complexity = random.choice(["basic", "intermediate", "advanced", "edge_case"])
            if complexity in self.complexity_variations and self.complexity_variations[complexity]:
                input_text += random.choice(self.complexity_variations[complexity])
            
            # Add pattern type variation
            pattern_type = random.choice(["explicit", "implicit", "contextual", "ambiguous"])
            if pattern_type in self.pattern_type_modifiers and self.pattern_type_modifiers[pattern_type]:
...
class EdgeCasePatternGenerator:
    ...
    def __init__(self):
        self.edge_cases = [
            ...
            ("URGENT: Production down", "meta-coordinator", ["infrastructure"], "hierarchical", "crisis_mode"),
            ("AsyncMock pytest configuration", "test-specialist", ["testing"], "sequential", "highly_specific"),
        ]
    ...
    def generate_patterns(self, count: int, domain: str = None) -> List[TestPattern]:
        ...
        for i in range(count):
            ...
            input_text, expected_agent, expected_domains, expected_coordination, edge_type = edge_case
            ...
            pattern = TestPattern(
                ...
                domain_category="mixed" if len(expected_domains) > 1 else (expected_domains[0] if expected_domains else "none"),
                pattern_type="ambiguous",
                validation_criteria={
                    "agent_match_required": False,
                    "domain_overlap_min": 0.5,
                    "coordination_match_required": False,
                    "acceptable_alternatives": ["digdeep", "meta-coordinator", "analysis-gateway"]
                },
                metadata={
...
class AgentSelectionValidator:
    ...
    def validate_pattern(self, pattern: TestPattern, conversation_history: List[str] = None) -> ValidationResult:
        ...
        try:
            ...
            actual_domains = list(details.get('context_analysis', {}).get('domains', {}).keys())
            actual_coordination = details.get('context_analysis', {}).get('coordination_pattern', '')
            
            accuracy_metrics = self._calculate_accuracy_metrics(pattern, selected_agent, actual_domains, actual_coordination)
            ...
            validation_status, failure_reasons = self._determine_validation_status(pattern, accuracy_metrics, confidence)
            ...
            return ValidationResult(
                ...
                accuracy_metrics={"overall_accuracy": 0.0, "agent_accuracy": 0.0, "domain_accuracy": 0.0},
                performance_metrics={"response_time_ms": 999999, "confidence_score": 0.0, "processing_efficiency": 0.0},
    ...
    def _calculate_accuracy_metrics(self, pattern: TestPattern, selected_agent: str, 
                                  actual_domains: List[str], actual_coordination: str) -> Dict[str, float]:
        """Calculate detailed accuracy metrics"""
        metrics = {}
        
        if selected_agent == pattern.expected_agent:
            metrics["agent_accuracy"] = 1.0
        elif selected_agent in pattern.validation_criteria.get("acceptable_alternatives", []):
        ...
        if not expected_domains:
            ...
            metrics["domain_accuracy"] = len(intersection) / len(union) if union else 0.0
    ...
    def _determine_validation_status(self, pattern: TestPattern, accuracy_metrics: Dict[str, float], 
                                   confidence: float) -> Tuple[str, List[str]]:
        """Determine overall validation status and failure reasons"""
        failure_reasons = []
        
        if pattern.validation_criteria.get("agent_match_required", True) and accuracy_metrics["agent_accuracy"] < 0.7:
...
class AgentSelectionTestFramework:
    ...
    def generate_comprehensive_test_suite(self, patterns_per_domain: int = 20, edge_cases: int = 15) -> List[TestPattern]:
        ...
        for domain in domains:
            ...
            logger.info(f"Generated {len(domain_patterns)} patterns for domain: {domain}")
    ...
    def run_comprehensive_validation(self, custom_patterns: List[TestPattern] = None) -> ComprehensiveValidationReport:
        ...
        logger.info(f"Comprehensive validation completed in {execution_time:.2f} seconds")
        return report
    
    def _generate_comprehensive_report(self, patterns: List[TestPattern], 
                                     validation_results: List[ValidationResult]) -> ComprehensiveValidationReport:
        ...
        overall_accuracy = statistics.mean([r.accuracy_metrics["overall_accuracy"] for r in validation_results])
        
        domain_coverage_score = self._calculate_domain_coverage_score(patterns, validation_results)
        pattern_type_performance = self._analyze_pattern_type_performance(validation_results)
        complexity_level_performance = self._analyze_complexity_level_performance(validation_results)
        edge_case_handling = self._analyze_edge_case_handling(validation_results)
        performance_benchmarks = self._calculate_performance_benchmarks(validation_results)
        statistical_significance = self._calculate_statistical_significance(validation_results)
        
        improvement_recommendations = self._generate_improvement_recommendations(
            overall_accuracy, domain_coverage_score, pattern_type_performance, 
            complexity_level_performance, edge_case_handling, performance_benchmarks
        )
        
        detailed_breakdowns = {
            "validation_results_summary": self._summarize_validation_results(validation_results),
            "domain_distribution": self._analyze_domain_distribution(patterns),
            "failure_analysis": self._analyze_failures(validation_results),
            "confidence_distribution": self._analyze_confidence_distribution(validation_results)
        }
    ...
    def _calculate_domain_coverage_score(self, patterns: List[TestPattern], 
        ...
        for pattern, result in zip(patterns, validation_results):
            domain_results[pattern.domain_category].append(result.accuracy_metrics["overall_accuracy"])
        ...
        if domain_accuracies:
            return len(domain_accuracies) / sum(1/acc if acc > 0 else float('inf') for acc in domain_accuracies)
        return 0.0
    
    def _analyze_pattern_type_performance(self, validation_results: List[ValidationResult]) -> Dict[str, Dict[str, float]]:
        ...
        for pattern_type, results in pattern_type_results.items():
            if results:
                performance[pattern_type] = {
                    "count": len(results),
                    "accuracy_mean": statistics.mean([r.accuracy_metrics["overall_accuracy"] for r in results]),
                    "response_time_mean": statistics.mean([r.performance_metrics["response_time_ms"] for r in results]),
                    "confidence_mean": statistics.mean([r.confidence_score for r in results]),
                    "pass_rate": sum(1 for r in results if r.validation_status == "pass") / len(results)
                }
        
        return performance
    
    def _analyze_complexity_level_performance(self, validation_results: List[ValidationResult]) -> Dict[str, Dict[str, float]]:
        ...
        for complexity, results in complexity_results.items():
            if results:
                performance[complexity] = {
                    ...
                    "accuracy_mean": statistics.mean([r.accuracy_metrics["overall_accuracy"] for r in results]),
                    "pass_rate": sum(1 for r in results if r.validation_status == "pass") / len(results),
                    "fail_rate": sum(1 for r in results if r.validation_status == "fail") / len(results)
                }
        
        return performance
    
    def _analyze_edge_case_handling(self, validation_results: List[ValidationResult]) -> Dict[str, float]:
        """Analyze edge case handling performance"""
        edge_case_results = [r for r in validation_results if r.test_pattern.complexity_level == "edge_case"]
        ...
        return {
            ...
            "edge_case_accuracy": statistics.mean([r.accuracy_metrics["overall_accuracy"] for r in edge_case_results]),
            "edge_case_pass_rate": sum(1 for r in edge_case_results if r.validation_status == "pass") / len(edge_case_results),
            "edge_case_confidence_mean": statistics.mean([r.confidence_score for r in edge_case_results]),
            "edge_case_response_time": statistics.mean([r.performance_metrics["response_time_ms"] for r in edge_case_results])
        }
    
    def _calculate_performance_benchmarks(self, validation_results: List[ValidationResult]) -> Dict[str, float]:
        """Calculate performance benchmarks"""
        response_times = [r.performance_metrics["response_time_ms"] for r in validation_results]
        ...
        return {
            ...
            "low_confidence_rate": sum(1 for c in confidence_scores if c < 0.7) / len(confidence_scores) if confidence_scores else 0.0
        }
    
    def _calculate_statistical_significance(self, validation_results: List[ValidationResult]) -> Dict[str, float]:
        """Calculate statistical significance metrics"""
        accuracy_scores = [r.accuracy_metrics["overall_accuracy"] for r in validation_results]
    ...
    def _generate_improvement_recommendations(self, overall_accuracy: float, domain_coverage: float,
                                            pattern_type_performance: Dict, complexity_performance: Dict,
                                            edge_case_handling: Dict, performance_benchmarks: Dict) -> List[str]:
        """Generate actionable improvement recommendations"""
        recommendations = []
        
        if overall_accuracy < 0.90:
            recommendations.append(f"Overall accuracy ({overall_accuracy:.2%}) is below 90% target. Focus on improving pattern matching algorithms.")
        elif overall_accuracy >= 0.95:
            recommendations.append(f"Excellent overall accuracy ({overall_accuracy:.2%}) achieved. Consider optimizing for speed and edge cases.")
        
        if domain_coverage < 0.85:
            recommendations.append(f"Domain coverage score ({domain_coverage:.2f}) indicates uneven performance across domains.")
        ...
        if not recommendations:
            recommendations.append("System performance meets all targets. Continue monitoring and fine-tuning as needed.")
    ...
    def _analyze_domain_distribution(self, patterns: List[TestPattern]) -> Dict[str, int]:
        """Analyze domain distribution in test patterns"""
        domain_counts = Counter(p.domain_category for p in patterns)
        return dict(domain_counts)
    
    def _analyze_failures(self, validation_results: List[ValidationResult]) -> Dict[str, Any]:
    ...
    def _analyze_confidence_distribution(self, validation_results: List[ValidationResult]) -> Dict[str, float]:
    ...
    def save_validation_report(self, report: ComprehensiveValidationReport, filename: str = None):
