...
class PerformanceGuard:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if duration_ms > self.thresholds.max_selection_time_ms:
            self.logger.warning(f"Performance threshold exceeded: {duration_ms:.2f}ms")
            return False
        return True
...
class PatternStore:
    ...
    def __init__(self, hub_path: Optional[str] = None):
        self.hub_path = hub_path or str(
            Path.cwd() / ".claude" / "memory" / "coordination-hub.md"
        )
            ...
            for line in pattern_lines:
                if line.startswith("- **") and "**:" in line:
                    try:
                        ...
                        confidence = float(
                            re.search(r"confidence: ([0-9.]+)", details).group(1)
                        )
                        exec_time = float(
                            re.search(r"execution_time: ([0-9.]+)", details).group(1)
                        )
                        days_ago = int(
                            re.search(r"learned: ([0-9]+)", details).group(1)
                        )
    ...
    def _generate_patterns_section(self) -> str:
        ...
        for domain, patterns in domain_patterns.items():
            ...
            sorted_patterns = sorted(
                patterns, key=lambda p: p.success_rate, reverse=True
            )
    ...
    def _get_avg_operation_time(self) -> float:
        ...
        if not self.performance_metrics:
            return 0.0
        return sum(m["duration_ms"] for m in self.performance_metrics) / len(
...
class EnhancedBoundaryDetector:
    ...
    def detect_domain_boundaries(self, query: str) -> List[DomainBoundary]:
        ...
        for domain_type, patterns in self.domain_patterns.items():
            ...
            for pattern in patterns["core_patterns"]:
                ...
                if matches:
                    ...
                    for match in matches:
                        position_weight = max(
                            1.0, 2.0 - (match.start() / len(query_lower))
                        )
            ...
            for pattern in patterns["boundary_indicators"]:
                ...
                if matches:
                    ...
                    weight_multiplier = (
                        1.8 if multi_domain_signals["has_coordination"] else 1.5
                    )
        ...
        detection_threshold = self._calculate_dynamic_threshold(
            multi_domain_signals, coordination_strength
        )
        ...
        if sorted_domains and sorted_domains[0][1]["total_score"] > detection_threshold:
            ...
            boundary = DomainBoundary(
                ...
                boundary_patterns=primary_data["matched_patterns"]
                + primary_data["boundary_patterns"],
                overlap_indicators=overlap_indicators,
                complexity_score=primary_data["complexity_score"]
                + len(secondary_domains) * 0.5,
            )
    ...
    def _calculate_dynamic_threshold(
        self, multi_domain_signals: Dict, coordination_strength: float
    ) -> float:
    ...
    def _identify_secondary_domains(
        self,
        sorted_domains: List,
        primary_data: Dict,
        multi_domain_signals: Dict,
        ...
        for domain, data in sorted_domains:
            ...
            if (
                data["total_score"] > secondary_threshold
                or data["boundary_score"] > 0.5
                or (
                    data["core_score"] > 0.2
                    and multi_domain_signals["has_coordination"]
                )
            ):
    ...
    def _calculate_calibrated_confidence(
        ...
        if base_score > 4.0:  # Very strong single domain signal
            confidence = max(
                confidence, self.confidence_calibration["single_strong_signal"]
            )
        ...
        if len(secondary_domains) > 0:
            if multi_domain_signals["has_coordination"] or coordination_strength > 0.5:
                ...
                confidence = max(
                    confidence, self.confidence_calibration["cross_domain_pattern"]
                )
            else:
                # Medium confidence for inferred multi-domain
                confidence = max(
                    confidence, self.confidence_calibration["multiple_weak_signals"]
                )

        # Boost for strong coordination signals
        if coordination_strength > 0.7:
            confidence = min(
                1.0, confidence + self.confidence_calibration["high_confidence_boost"]
            )

        # Multi-domain bonus
        if len(secondary_domains) > 1:
            confidence = min(
                1.0, confidence + self.confidence_calibration["multi_domain_bonus"]
            )

        return confidence

    def _detect_enhanced_overlap_indicators(
        self,
        query: str,
        primary: DomainType,
        secondary: List[DomainType],
        multi_domain_signals: Dict,
    ) -> List[str]:
        ...
        domain_combinations = {
            (
                DomainType.TESTING,
                DomainType.INFRASTRUCTURE,
            ): "testing_infrastructure_integration",
            (
                DomainType.SECURITY,
                DomainType.PERFORMANCE,
            ...
            (
                DomainType.DOCUMENTATION,
                DomainType.TESTING,
            ): "documentation_testing_coordination",
            (
                DomainType.INFRASTRUCTURE,
                DomainType.SECURITY,
    ...
    def _detect_overlap_indicators(
        self, query: str, primary: DomainType, secondary: List[DomainType]
    ) -> List[str]:
    ...
    def _analyze_priority_context(self, query: str) -> Dict[str, any]:
        ...
        high_count = sum(
            1 for indicator in priority_indicators["high"] if indicator in query_lower
        )
        medium_count = sum(
            1 for indicator in priority_indicators["medium"] if indicator in query_lower
        )
        low_count = sum(
            1 for indicator in priority_indicators["low"] if indicator in query_lower
        )
...
class ConflictDetectionEngine:
    ...
    def _build_conflict_patterns(self) -> Dict[ConflictType, Dict[str, any]]:
        ...
        return {
            ...
            ConflictType.QUALITY_SPEED: {
                "domain_pairs": [
                    (DomainType.CODE_QUALITY, DomainType.PERFORMANCE),
            ...
            ConflictType.APPROACH_CONTRADICTION: {
                "domain_pairs": [
                    (DomainType.INFRASTRUCTURE, DomainType.SECURITY),
            ...
            ConflictType.DEPENDENCY_CONFLICT: {
                "domain_pairs": [
                    (DomainType.INFRASTRUCTURE, DomainType.TESTING),
    ...
    def detect_conflicts(
        self, boundaries: List[DomainBoundary], query: str
        ...
        for conflict_type, pattern_config in self.conflict_patterns.items():
            ...
            for domain_pair in pattern_config["domain_pairs"]:
                if (
                    domain_pair[0] in all_domains and domain_pair[1] in all_domains
                ) or (domain_pair[1] in all_domains and domain_pair[0] in all_domains):
                    ...
                    if indicator_matches > 0:
                        ...
                        if factor_count > 0:
                            severity = min(
                                1.0, severity + min(0.15, factor_count * 0.05)
                            )

                        # Adjust severity based on number of indicator matches (more conservative)
                        if indicator_matches > 1:
                            severity = min(
                                1.0, severity + (indicator_matches - 1) * 0.05
                            )

                        # Add domain-specific severity boost
                        severity = self._apply_domain_severity_boost(
                            severity, domain_pair, query_lower
                        )
                        ...
                        conflict = ConflictDetection(
                            ...
                            affected_agents=self._get_affected_agents(
                                list(domain_pair)
                            ),
                        )
    ...
    def _apply_domain_severity_boost(
        self,
        base_severity: float,
        ...
        if (
            domain_pair in critical_pairs
            or tuple(reversed(domain_pair)) in critical_pairs
        ):
            severity = min(1.0, severity + 0.2)
    ...
    def _detect_resource_conflicts(
        self, domains: List[DomainType], query: str
    ...
    def _deduplicate_conflicts(
        ...
        for conflict in conflicts:
            ...
            if (
                conflict_key not in seen_conflicts
                or conflict.severity > seen_conflicts[conflict_key].severity
            ):
...
class PatternLearningEngine:
    ...
    def __init__(self, coordination_hub_path: Optional[str] = None):
        ...
        self.successful_patterns = defaultdict(
            list
        )  # query_type -> [(pattern, success_rate)]
        ...
        self.learning_threshold = (
            0.75  # Minimum success rate to consider pattern successful
        )
        self.pattern_weights = defaultdict(float)  # pattern -> weight
        self.coordination_hub_path = (
            coordination_hub_path or self._get_coordination_hub_path()
        )
        ...
        if confidence >= self.learning_threshold:
            ...
            if query_type:
                ...
                self.pattern_weights[pattern_key] += (
                    0.1 if user_feedback is True else 0.05
                )
                ...
                if (
                    total_patterns > 0 and total_patterns % 5 == 0
                ):  # Store every 5 successful patterns
                    self.store_successful_patterns_to_hub()

    def learn_from_failure(
        self, query: str, selected_agent: str, expected_agent: str, reasons: List[str]
    ):
    ...
    def get_learning_stats(self) -> Dict[str, any]:
        ...
        total_failures = sum(
            len(patterns) for patterns in self.failed_patterns.values()
        )
    ...
    def _load_existing_patterns(self):
        ...
        try:
            ...
            logger.warning(
                f"Could not load existing patterns from coordination hub: {e}"
            )
                    ...
                    try:
                        ...
                        if end_idx > start_idx:
                            ...
                            if "confidence:" in stripped_line:
                                conf_start = stripped_line.find("confidence: ") + len(
                                    "confidence: "
                                )
                                ...
                                if conf_end > conf_start:
                                    ...
                                    try:
                                        ...
                                        if (
                                            confidence >= 0.6
                                        ):  # Only load patterns with reasonable confidence
                                            # Convert confidence to pattern weight with slight boost for persistence
                                            self.pattern_weights[pattern_key] = (
                                                confidence * 0.6
                                            )
                                            logger.debug(
                                                f"Loaded pattern: {pattern_key} with weight {confidence * 0.6:.3f}"
                                            )
                                    except ValueError:
                                        logger.warning(
                                            f"Could not parse confidence value: {confidence_str}"
                                        )
                    except (IndexError, ValueError) as e:
                        logger.debug(
                            f"Could not parse pattern line: {line.strip()}, error: {e}"
                        )
    ...
    def store_successful_patterns_to_hub(self):
        ...
        try:
            ...
            updated_content = self._update_patterns_in_content(
            ...
            logger.info(
                f"Stored {len(self.successful_patterns)} successful pattern types to coordination hub"
            )
    ...
    def _generate_patterns_section(self) -> str:
        ...
        for query_type, patterns in self.successful_patterns.items():
            ...
            section_lines.append(
                f"**{query_type.title().replace('_', ' ')} Patterns:**"
            )

            # Sort patterns by success rate
            sorted_patterns = sorted(
                patterns, key=lambda x: x.get("confidence", 0), reverse=True
            )
    ...
    def _update_patterns_in_content(self, content: str, patterns_section: str) -> str:
        ...
        if pattern_start > -1:
            ...
            if next_section > -1:
                ...
                return (
                    content[:pattern_start] + patterns_section + content[next_section:]
                )
            else:
...
class EnhancedCrossDomainCoordinator:
    ...
    def analyze_cross_domain_integration(self, query: str) -> CrossDomainAnalysis:
        ...
        try:
            ...
            if (
                hasattr(self, "pattern_learning_engine")
                and self.pattern_learning_engine
            ):
                learned_suggestion = (
                    self.pattern_learning_engine.get_learned_agent_suggestion(query)
                )
            ...
            coordination_recommendation = self._generate_coordination_recommendation(
                boundaries, conflicts, query
            )

            # Step 5: Generate agent suggestions with conflict awareness and learning integration
            agent_suggestions = self._generate_agent_suggestions_with_learning(
                boundaries, conflicts, learned_suggestion
            )
            ...
            if is_success:
                ...
                if task_success is True:
                    effective_confidence = min(
                        1.0, confidence + 0.1
                    )  # Boost for confirmed success
                self.pattern_learning_engine.learn_from_success(
                    query, selected_agent, effective_confidence, user_feedback
                )
    ...
    def get_learning_insights(self) -> Dict[str, any]:
        ...
        if (
            not hasattr(self, "pattern_learning_engine")
            or not self.pattern_learning_engine
        ):
        ...
        coordinator_stats = {
            "analyses_with_infrastructure": len(
                [
                    a
                    for a in getattr(self, "analysis_history", [])
                    if any(
                        b.primary_domain == DomainType.INFRASTRUCTURE
    ...
    def _generate_coordination_recommendation(
        ...
        if len(boundary.secondary_domains) == 1:
            ...
            coordination_strategies.append(
                "Strategic meta-coordination with comprehensive orchestration"
            )
        ...
        if conflicts:
            ...
            if high_severity_conflicts:
                coordination_strategies.append(
                    "High-priority conflict resolution required"
                )

            for conflict in conflicts:
                if conflict.conflict_type == ConflictType.TIMING_CONFLICT:
                    coordination_strategies.append(
                        "Sequential coordination with timing management"
                    )
                elif conflict.conflict_type == ConflictType.RESOURCE_COMPETITION:
                    coordination_strategies.append(
                        "Resource-aware coordination scheduling"
                    )
        ...
        if (
            any(complex_triggers)
            and "meta" not in recommendation.lower()
            and "strategic" not in recommendation.lower()
        ):
            recommendation = "Strategic meta-coordination + " + recommendation

        return recommendation

    def _generate_agent_suggestions_with_learning(
        self,
        boundaries: List[DomainBoundary],
        conflicts: List[ConflictDetection],
        learned_suggestion: Optional[Tuple[str, float]],
        ...
        if boundary.primary_domain == DomainType.INFRASTRUCTURE:
            ...
            primary_agent, primary_confidence = self._get_domain_agent(
            ...
            if not learned_suggestion or learned_suggestion[0] != primary_agent:
                ...
                suggestions.append(
                    (primary_agent, primary_confidence * boundary.confidence)
                )
        else:
            ...
            if (
                learned_suggestion
                and learned_suggestion[1] > 0.75
                and self._is_learned_agent_suitable_for_domain(
                    learned_suggestion[0], boundary.primary_domain
                )
            ):
        ...
        for secondary_domain in boundary.secondary_domains:
            ...
            if (
                secondary_domain == DomainType.INFRASTRUCTURE
                and learned_suggestion
                and learned_suggestion[0] == agent
                and learned_suggestion[1] > 0.6
            ):
        ...
        if len(boundary.secondary_domains) >= 4 or any(
            c.severity >= 0.8 for c in conflicts
        ):
        ...
        for agent, confidence in suggestions:
            if (
                agent not in unique_suggestions
                or confidence > unique_suggestions[agent]
            ):
    ...
    def _generate_agent_suggestions(
        self, boundaries: List[DomainBoundary], conflicts: List[ConflictDetection]
    ) -> List[Tuple[str, float]]:
        """Legacy method - redirects to learning-enhanced version."""
        return self._generate_agent_suggestions_with_learning(
            boundaries, conflicts, None
        )
    ...
    def _is_learned_agent_suitable_for_domain(
        self, agent_name: str, domain: DomainType
    ) -> bool:
    ...
    def get_analysis_stats(self) -> Dict[str, any]:
        ...
        avg_processing_time = sum(
            a.processing_time_ms for a in self.analysis_history
        ...
        return {
            ...
            "conflict_detection_rate": len(
                [a for a in self.analysis_history if a.potential_conflicts]
            )
            / len(self.analysis_history),
        }
...
class LearningCoordinator(EnhancedCrossDomainCoordinator):
    ...
    def __init__(self, pattern_store: PatternStore, thresholds: SafetyThresholds):
        super().__init__()
    ...
    def _enhance_with_patterns(self, result: CoordinationResult) -> CoordinationResult:
            ...
            if pattern_id in self.pattern_store.patterns:
                ...
                if pattern.success_rate > result.confidence:
                    ...
                    if enhancement_time > self.thresholds.max_pattern_lookup_ms:
                        logger.warning(
                            f"Pattern enhancement too slow: {enhancement_time:.2f}ms"
                        )
                        return result
...
class EvolutionEngine:
    ...
    def evolve_patterns(self, query_type: str) -> List[CoordinationPattern]:
        ...
        type_patterns = [
            p for p in patterns.values() if query_type in p.query_signature
        ]
        ...
        while generation < self.max_generations:
            ...
            elite_patterns = sorted(
                type_patterns, key=lambda p: p.success_rate, reverse=True
            )[:elite_count]
            ...
            evolved_patterns = [
                p for p in new_generation if p.success_rate >= self.evolution_threshold
            ]
    ...
    def _select_parent(
        self, patterns: List[CoordinationPattern]
    ) -> CoordinationPattern:
    ...
    def _crossover_patterns(
        self, p1: CoordinationPattern, p2: CoordinationPattern
    ) -> CoordinationPattern:
        ...
        new_sequence = (
            p1.agent_sequence[:crossover_point] + p2.agent_sequence[crossover_point:]
        )
        ...
        return CoordinationPattern(
            ...
            query_signature=self._combine_signatures(
                p1.query_signature, p2.query_signature
            ),
    ...
    def _mutate_pattern(self, pattern: CoordinationPattern) -> CoordinationPattern:
        ...
        if mutation_type == "add" and len(sequence) < 5:
            ...
            new_agent = random.choice(
                ["test-specialist", "security-enforcer", "performance-optimizer"]
            )
    ...
    def get_evolution_stats(self) -> Dict[str, any]:
        ...
        return {
            ...
            "success_rate_improvement": self.history[-1]["avg_success_rate"]
            - self.history[0]["avg_success_rate"],
            "avg_generation_size": sum(gen["pattern_count"] for gen in self.history)
            / generations,
...
class MemoryArchitecture:
    ...
    def store_pattern(self, pattern: CoordinationPattern) -> bool:
        ...
        try:
            ...
            if operation_time > 150:  # 150ms limit
                logger.warning(
                    f"Pattern storage time exceeded limit: {operation_time:.2f}ms"
                )
                return False
    ...
    def load_patterns(
        ...
        try:
            ...
            # Update cache
            self.cache[cache_key] = {"data": patterns, "timestamp": time.time()}
    ...
    def get_performance_stats(self) -> Dict[str, float]:
        ...
        return {
            "avg_operation_time_ms": sum(self.operation_times)
...
class EnhancedCrossDomainCoordinator:
    ...
    def _calculate_integration_complexity(
        ...
        if conflicts:
            ...
            if any(
                c.conflict_type == ConflictType.SECURITY_PERFORMANCE for c in conflicts
            ):
                complexity += 0.1
            if any(
                c.conflict_type == ConflictType.RESOURCE_COMPETITION for c in conflicts
            ):
                complexity += 0.1
        ...
        if (
            boundary.primary_domain == DomainType.TESTING
            and not conflicts
            and not boundary.secondary_domains
        ):
    ...
    def _generate_selection_reasoning(
        ...
        query: str,
        boundaries: List[DomainBoundary],
        suggestion: Optional[Tuple[str, float]] = None,
    ) -> str:
        ...
        if boundary.secondary_domains:
            domain_reason += (
                f" with {len(boundary.secondary_domains)} secondary domains"
            )
...
class CrossDomainOptimizer:
    ...
    def optimize_domain_coordination(
        self, query: str, boundaries: List[DomainBoundary]
        ...
        try:
            ...
            best_pattern = self._select_optimal_pattern(
                patterns, boundaries[0], expertise
            )
            if not best_pattern:
                return None
    ...
    def _get_optimization_pattern_key(self, query: str) -> Optional[str]:
        ...
        if any(
            term in query_lower for term in ["auth", "authentication", "authorization"]
        ):
            return "auth"
        elif any(
            term in query_lower for term in ["parallel", "concurrent", "simultaneous"]
        ):
            return "parallel"
        elif any(
            term in query_lower for term in ["security", "secure", "vulnerability"]
        ):
            return "security"
        elif any(
            term in query_lower
            for term in ["infrastructure", "deployment", "container"]
        ):
    ...
    def _select_optimal_pattern(
        self, patterns: List[str], boundary: DomainBoundary, expertise: Dict[str, float]
    ) -> Optional[str]:
        ...
        for pattern in patterns:
            score = self._score_pattern(
                pattern, primary_domain, secondary_domains, expertise
            )
    ...
    def _score_pattern(
        ...
        matched_secondaries = sum(
            1 for d in pattern_domains[1:] if d in secondary_domains
        )
    ...
    def _calculate_confidence(
        self, agent_sequence: List[str], expertise: Dict[str, float]
    ) -> float:
    ...
    def get_optimization_stats(self) -> Dict[str, any]:
        ...
        for pattern_key, metrics in self.performance_metrics.items():
            ...
            stats[pattern_key] = {
                ...
                "current_accuracy": self.domain_expertise[pattern_key][
                    "current_accuracy"
                ],
        ...
        try:
            ...
            if metrics:
                if metrics["success_rate"] < self.min_success_rate:
                    logger.warning(
                        f"Success rate below threshold: {metrics['success_rate']:.2f}"
                    )
                    return False
...
class HealthMonitor:
    ...
    def check_system_health(self, coordinator: LearningCoordinator) -> bool:
        ...
        try:
            ...
            if not system_healthy:
                logger.warning(
                    "System health check failed - see health metrics for details"
                )
        ...
        try:
            ...
            if store_stats:
                if store_stats["avg_operation_time_ms"] > 150:
                    logger.warning(
                        f"Slow pattern operations: {store_stats['avg_operation_time_ms']:.2f}ms"
                    )
                    return False

                if store_stats["max_operation_time_ms"] > 500:
                    logger.warning(
                        f"Pattern operation timeout: {store_stats['max_operation_time_ms']:.2f}ms"
                    )
                    return False
    ...
    def _check_pattern_health(self, coordinator: LearningCoordinator) -> bool:
        ...
        try:
            ...
            now = datetime.now()
            stale_patterns = sum(
                1 for p in patterns.values() if (now - p.last_used).days > 30
            )
    ...
    def get_health_report(self) -> Dict[str, any]:
        ...
        return {
            "system_availability": sum(1 for h in recent_health if h["healthy"])
            / len(recent_health),
            "component_health": {
                "coordinator": sum(1 for h in recent_health if h["coordinator_healthy"])
                / len(recent_health),
                "memory": sum(1 for h in recent_health if h["memory_healthy"])
                / len(recent_health),
                "patterns": sum(1 for h in recent_health if h["pattern_healthy"])
                / len(recent_health),
            },
        ...
        try:
            ...
            for line in pattern_lines:
                if line.startswith("- **") and "**:" in line:
                    ...
                    if not all(
                        x in line for x in ["confidence:", "keywords:", "learned:"]
                    ):
                        return False
    ...
    def _update_patterns_section(
        self, content: str, new_pattern: CoordinationPattern
    ) -> str:
