...
class EnhancedPatternLearningEngine(PatternLearningEngine):
    ...
    def _parse_single_agent(self, agent_file: str) -> Optional[AgentProfile]:
        ...
        try:
            ...
            specialization_score = self._calculate_specialization_score(keywords, capabilities)
    ...
    def _extract_trigger_keywords(self, description: str, content: str = "") -> List[str]:
        ...
        for text in text_sources:
            ...
            domain_keywords = [
                ...
                'unittest', 'validation', 'assert', 'failure', 'failures',
                ...
                'authentication', 'authorization', 'encryption', 'hardening',
                ...
                'architecture', 'pattern', 'design', 'lint',
                
                # Documentation domain
                'documentation', 'guide', 'readme', 'technical', 'writing',
                'api', 'specification', 'manual'
            ]
    ...
    def _extract_capabilities(self, content: str) -> List[str]:
        ...
        for pattern in capability_patterns:
            ...
            for match in matches:
                cleaned = match.strip()
                if len(cleaned) > 3 and len(cleaned) < 50:  # Reasonable capability length
    ...
    def _extract_trigger_patterns(self, content: str) -> List[str]:
        ...
        perfect_match = re.search(r'Perfect.*?for[^.]*\.', content, re.DOTALL | re.IGNORECASE)
        if perfect_match:
            patterns.append(perfect_match.group(0))
        
        # Look for "Use PROACTIVELY when" patterns
        proactive_match = re.search(r'Use PROACTIVELY.*?\.',  content, re.DOTALL | re.IGNORECASE)
        if proactive_match:
            patterns.append(proactive_match.group(0))
            
        # Look for "Auto-Activate UltraThink when detecting" patterns
        ultrathink_pattern = re.search(r'Auto-Activate UltraThink when detecting:.*?(?=###|##|$)', content, re.DOTALL | re.IGNORECASE)
        if ultrathink_pattern:
            patterns.append(ultrathink_pattern.group(0))
            
        return patterns
    
    def _calculate_specialization_score(self, keywords: List[str], capabilities: List[str]) -> float:
        ...
        technical_keywords = {
            'pytest', 'asyncmock', 'kubernetes', 'docker', 'semgrep', 'oauth',
            'encryption', 'vulnerability', 'orchestration', 'profiling'
        }
    ...
    def get_enhanced_agent_suggestion(self, query: str) -> Optional[Tuple[str, float]]:
        ...
        for agent_name, profile in self.agent_profiles.items():
            ...
            if keyword_matches > 0:
                keyword_score = keyword_matches / len(profile.keywords) if profile.keywords else 0
    ...
    def get_learning_enhancement_stats(self) -> Dict[str, any]:
        ...
        try:
            base_stats = self.get_learning_stats() if hasattr(self, 'get_learning_stats') else {}
        except Exception:
            base_stats = {}
        
        enhancement_stats = {
            'agent_profiles_loaded': len(self.agent_profiles),
            'total_keywords_extracted': sum(len(profile.keywords) for profile in self.agent_profiles.values()),
            'total_capabilities_extracted': sum(len(profile.capabilities) for profile in self.agent_profiles.values()),
            'average_specialization_score': sum(profile.specialization_score for profile in self.agent_profiles.values()) / max(len(self.agent_profiles), 1),
            'agents_with_trigger_patterns': sum(1 for profile in self.agent_profiles.values() if profile.trigger_patterns)
        }
