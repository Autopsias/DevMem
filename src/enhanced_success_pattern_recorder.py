...
class EnhancedSuccessPatternRecorder:
    ...
    def record_successful_usage(
        self, query: str, selected_agent: str, success_metrics: Dict
    ) -> bool:
    ...
    def _generate_pattern_key(self, query: str, agent: str) -> str:
        ...
        if any(
            kw in query_lower for kw in ["test", "pytest", "mock", "coverage", "async"]
        ):
            return "testing_patterns"
        elif any(
            kw in query_lower
            for kw in ["docker", "container", "kubernetes", "infrastructure"]
        ):
            return "container_patterns"
        elif any(
            kw in query_lower
            for kw in ["security", "vulnerability", "audit", "compliance"]
        ):
            return "security_patterns"
        elif any(
            kw in query_lower
            for kw in ["performance", "optimization", "bottleneck", "resource"]
        ):
            return "performance_patterns"
        elif any(
            kw in query_lower for kw in ["documentation", "docs", "guide", "readme"]
        ):
            return "documentation_patterns"
        elif any(
            kw in query_lower
            for kw in ["quality", "refactoring", "architecture", "pattern"]
        ):
    ...
    def _update_learning_section(self, pattern_entry: Dict) -> bool:
        ...
        try:
            ...
            if not os.path.exists(self.coordination_hub_path):
                logger.warning(
                    f"Coordination hub not found: {self.coordination_hub_path}"
                )
                return False
            ...
            updated_content = self._insert_pattern_in_section(
                content, pattern_entry, pattern_line
            )
    ...
    def _insert_pattern_in_section(
        self, content: str, pattern_entry: Dict, pattern_line: str
    ) -> str:
        ...
        target_section = section_mapping.get(
            pattern_type, "**Testing & Quality Assurance Patterns:**"
        )
        ...
        if section_start != -1:
            ...
            if next_subsection != -1 and (
                next_main_section == -1 or next_subsection < next_main_section
            ):
            ...
            logger.info(
                f"Target section {target_section} not found, attempting to add it"
            )
            return self._add_new_section(content, target_section, pattern_line)

    def _add_new_section(
        self, content: str, section_name: str, pattern_line: str
    ) -> str:
        ...
        if high_confidence_start != -1:
            ...
            medium_confidence_start = content.find(
                "### Medium-Confidence Learned Patterns", high_confidence_start
            )
    ...
    def get_recorded_patterns_count(self) -> int:
        ...
        try:
            ...
            learning_section_start = content.find(
            ...
            learning_content = (
                content[learning_section_start:next_main_section]
                if next_main_section != -1
                else content[learning_section_start:]
            )

            # Count pattern lines
            pattern_lines = [
                line
                for line in learning_content.split("\n")
