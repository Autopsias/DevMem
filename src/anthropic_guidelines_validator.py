...
class AnthropicGuidelinesValidator:
    ...
    def validate_learning_pattern(self, pattern: Dict) -> ValidationResult:
        ...
        capability_match_score = self._validate_agent_capability_match(
            agent_name, keywords
        )
        if capability_match_score < 0.7:
            issues.append(
                f"Agent {agent_name} may not be optimal for keywords: {keywords}"
            )
            recommendations.append(
                f"Consider reviewing agent selection for pattern {pattern_key}"
            )
        ...
        if len(keywords) < 2:
            ...
            recommendations.append(
                "Add more specific keywords to improve pattern accuracy"
            )
        ...
        if confidence < 0.7:
            issues.append(
                f"Pattern confidence {confidence:.2f} is below recommended threshold"
            )
            recommendations.append(
                "Only record patterns with confidence >= 0.7 for reliable learning"
            )
        ...
        if len(keywords) > 8:
            ...
            recommendations.append(
                "Focus on 3-5 core keywords for better pattern generalization"
            )
            compliance_score -= 0.1
        ...
        if self._has_circular_dependency(pattern_key, agent_name):
            ...
            recommendations.append(
                "Review coordination flow to avoid agent selection loops"
            )
    ...
    def _validate_agent_capability_match(
        self, agent_name: str, keywords: List[str]
    ) -> float:
        ...
        keyword_matches = sum(
            ...
            if any(
                keyword in capability or capability in keyword
        ...
        strong_matches = sum(
            1 for keyword in keywords if keyword in expected_capabilities
        )
    ...
    def validate_pattern_collection(self, patterns: List[Dict]) -> Dict[str, any]:
        ...
        for issue in all_issues:
            ...
            issue_frequency[normalized_issue] = (
                issue_frequency.get(normalized_issue, 0) + 1
            )
        ...
        return {
            ...
            "common_issues": [
                {"issue": issue, "frequency": freq} for issue, freq in common_issues
            ],
            "overall_compliance": compliance_rate
            >= self.compliance_thresholds["acceptable"],
            "anthropic_guidelines_met": compliance_rate
            >= 0.8,  # 80% minimum for full compliance
        }
...
"""
        ...
        for category, count in validation_results["category_distribution"].items():
            percentage = (
                count / validation_results["total_patterns"] * 100
                if validation_results["total_patterns"] > 0
                else 0
            )
            report += (
                f"- **{category.title()}**: {count} patterns ({percentage:.1f}%)\n"
            )
        ...
        if validation_results["compliance_rate"] < 0.8:
            ...
            report += (
                "- Current patterns show good compliance with Anthropic guidelines\n"
            )
            report += (
                "- Continue monitoring pattern quality and agent matching accuracy\n"
            )
