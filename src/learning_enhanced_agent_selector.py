...
class LearningEnhancedAgentSelector:
    ...
    def _initialize_learning_components(self):
        ...
        try:
            if EnhancedPatternLearningEngine:
                self.learning_engine = EnhancedPatternLearningEngine(
                    self.coordination_hub_path
                )
                logger.info(
                    f"Initialized learning engine with {len(self.learning_engine.agent_profiles)} agent profiles"
                )
    ...
    def _learning_enhanced_selection(
        self, query: str, context: Optional[Dict] = None
            ...
            if suggestion:
                ...
                if self.guidelines_validator:
                    pattern = {
                        ...
                        "keywords": (
                            self.pattern_recorder._extract_query_keywords(query)
                            if self.pattern_recorder
                            else []
                        ),
                    }
                    validation_result = (
                        self.guidelines_validator.validate_learning_pattern(pattern)
                    )
                ...
                if learning_boost > 0:
                    reasoning += (
                        f", boosted by {learning_boost:.3f} due to specialization"
                    )
    ...
    def _fallback_selection(
        self, query: str, context: Optional[Dict], start_time: float
    ) -> AgentSelectionResult:
    ...
    def record_selection_feedback(
        ...
        try:
            success_metrics = {
                "confidence": confidence,
                "indicators": (
                    ["successful_selection"] if success else ["failed_selection"]
                ),
            }
            ...
            return self.pattern_recorder.record_successful_usage(
                query, selected_agent, success_metrics
            )
    ...
    def _update_learning_stats(self, result: AgentSelectionResult):
        ...
        if result.learning_applied and result.learning_confidence_boost > 0:
            ...
            new_avg = (
                (current_avg * (total_enhanced - 1)) + result.learning_confidence_boost
            ) / total_enhanced
    ...
    def get_selection_stats(self) -> Dict[str, any]:
        ...
        if stats["total_selections"] > 0:
            stats["learning_usage_rate"] = (
                stats["learning_enhanced_selections"] / stats["total_selections"]
            )
            stats["fallback_rate"] = (
                stats["fallback_selections"] / stats["total_selections"]
            )
        else:
        ...
        if self.guidelines_validator:
            stats["anthropic_compliance_thresholds"] = (
                self.guidelines_validator.get_compliance_thresholds()
            )
    ...
    def validate_learning_system(self) -> Dict[str, any]:
        ...
        if self.pattern_recorder:
            validation["coordination_hub_valid"] = (
                self.pattern_recorder.validate_coordination_hub_format()
            )

        # Count loaded agent profiles
        if self.learning_engine:
            validation["agent_profiles_loaded"] = len(
                self.learning_engine.agent_profiles
            )

        # Overall system health
        validation["system_health"] = (
            validation["learning_engine_available"]
            and validation["pattern_recorder_available"]
            and validation["coordination_hub_valid"]
            and validation["agent_profiles_loaded"] > 0
        )

        return validation

    def test_learning_accuracy(
        ...
        for query, expected_agent in test_cases:
            ...
            if result.agent_name == expected_agent:
                ...
                self.record_selection_feedback(
                    query, result.agent_name, result.confidence_score, True
                )
