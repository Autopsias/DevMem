...
class SemanticAnalyzer:
    ...
    def _build_domain_semantics(self) -> Dict[str, Dict[str, List[str]]]:
        ...
        return {
            'testing': {
                'core_concepts': ['test', 'spec', 'assertion', 'mock', 'fixture', 'coverage', 'validation'],
                'actions': ['run', 'execute', 'validate', 'verify', 'check', 'assert', 'mock'],
                'quality_indicators': ['comprehensive', 'thorough', 'complete', 'robust', 'reliable'],
                'problem_indicators': ['failing', 'broken', 'incomplete', 'flaky', 'unstable']
            },
            'infrastructure': {
                'core_concepts': ['docker', 'container', 'deployment', 'orchestration', 'scaling', 'networking'],
                'actions': ['deploy', 'orchestrate', 'scale', 'configure', 'manage', 'monitor'],
                'quality_indicators': ['scalable', 'resilient', 'efficient', 'stable', 'robust'],
                'problem_indicators': ['failing', 'unstable', 'slow', 'bottleneck', 'resource']
            },
            'security': {
                'core_concepts': ['authentication', 'authorization', 'encryption', 'vulnerability', 'compliance'],
                'actions': ['secure', 'encrypt', 'authenticate', 'authorize', 'validate', 'audit'],
                'quality_indicators': ['secure', 'compliant', 'validated', 'encrypted', 'protected'],
                'problem_indicators': ['vulnerable', 'insecure', 'exposed', 'breach', 'weak']
            },
            'performance': {
                'core_concepts': ['latency', 'throughput', 'bottleneck', 'optimization', 'resource', 'memory'],
                'actions': ['optimize', 'tune', 'improve', 'accelerate', 'monitor', 'profile'],
                'quality_indicators': ['fast', 'efficient', 'optimized', 'scalable', 'responsive'],
                'problem_indicators': ['slow', 'bottleneck', 'inefficient', 'memory leak', 'high latency']
            }
        }
    ...
    def _build_context_indicators(self) -> Dict[str, List[str]]:
        ...
        return {
            'critical': ['critical', 'essential', 'vital', 'crucial', 'mandatory', 'required'],
    ...
    def analyze_semantic_patterns(self, text: str) -> Dict[str, Any]:
        ...
        for domain, patterns in self.domain_semantics.items():
            ...
            if score > 0:
                results['domain_scores'][domain] = {
                    'score': score,
                    'matches': matches,
                    'confidence': min(score / 8.0, 1.0)  # Adjusted normalization for better sensitivity
                }
        ...
        results['semantic_complexity'] = min((domain_count * 0.3 + relationship_count * 0.2), 1.0)
...
class CrossDomainRelationshipMapper:
    ...
    def find_domain_path(self, from_domain: str, to_domain: str, max_hops: int = 3) -> List[str]:
    ...
    def get_related_domains(self, domain: str, min_strength: float = 0.3) -> List[Tuple[str, float]]:
    ...
    def update_relationship_strength(self, from_domain: str, to_domain: str, 
                                   success: bool, learning_rate: float = 0.1):
...
class ContextPreservationManager:
    ...
    def preserve_context(self, context_elements: List[ContextElement], 
    ...
    def _full_transfer(self, context_elements: List[ContextElement], 
                      from_domain: str, to_domain: str) -> List[ContextElement]:
        """Transfer all context elements."""
        return context_elements.copy()
    
    def _selective_transfer(self, context_elements: List[ContextElement], 
                           from_domain: str, to_domain: str) -> List[ContextElement]:
        """Transfer only domain-relevant context elements."""
        relationship_strength = self.relationship_mapper.get_relationship_strength(from_domain, to_domain)
    ...
    def _hierarchical_transfer(self, context_elements: List[ContextElement], 
                              from_domain: str, to_domain: str) -> List[ContextElement]:
        """Transfer context using hierarchical importance structure."""
        # Sort by importance
        sorted_elements = sorted(context_elements, key=lambda x: x.importance, reverse=True)
    ...
    def _adaptive_transfer(self, context_elements: List[ContextElement], 
                          from_domain: str, to_domain: str) -> List[ContextElement]:
        """Adaptively select context preservation strategy based on context analysis."""
        # Analyze context characteristics
        total_elements = len(context_elements)
        avg_importance = sum(e.importance for e in context_elements) / max(total_elements, 1)
    ...
    def _calculate_preservation_metrics(self, original: List[ContextElement], 
                                       preserved: List[ContextElement],
                                       from_domain: str, to_domain: str,
                                       start_time: float) -> ContextPreservationMetrics:
    ...
    def get_preservation_quality_score(self, metrics: ContextPreservationMetrics) -> float:
        ...
        critical_preservation = (
            metrics.critical_elements_preserved / max(metrics.critical_elements_total, 1)
            if metrics.critical_elements_total > 0 else 1.0
        )
...
class DomainSpecificOptimizer:
    ...
    def optimize_for_domain(self, domain: str, coordination_request: str, 
        ...
        request_analysis = self._analyze_coordination_request(coordination_request, domain_config)
        
        # Select optimal agents
        optimal_agents = self._select_optimal_agents(available_agents, domain, request_analysis)
        
        # Determine coordination strategy
        coordination_strategy = self._determine_coordination_strategy(domain_config, request_analysis)
        
        # Calculate optimization confidence
        confidence = self._calculate_optimization_confidence(domain, request_analysis, optimal_agents)
    ...
    def _analyze_coordination_request(self, request: str, domain_config: Dict[str, Any]) -> Dict[str, Any]:
        """Analyze coordination request for domain-specific patterns."""
        request_lower = request.lower()
        
        # Check for priority keywords
        priority_matches = [
            keyword for keyword in domain_config['priority_keywords'] 
            if keyword in request_lower
        ]
        
        # Detect complexity indicators
        complexity_indicators = [
            'comprehensive', 'complex', 'multi-step', 'end-to-end', 
            'systematic', 'thorough', 'complete'
        ]
        complexity_matches = [ind for ind in complexity_indicators if ind in request_lower]
        
        # Detect parallel coordination signals
        parallel_indicators = [
            'parallel', 'simultaneously', 'coordinate', 'multiple', 
            'tasks in parallel', 'concurrent'
        ]
        parallel_matches = [ind for ind in parallel_indicators if ind in request_lower]
        
        return {
            'priority_keyword_matches': priority_matches,
            'priority_score': len(priority_matches) / len(domain_config['priority_keywords']),
    ...
    def _select_optimal_agents(self, available_agents: List[str], domain: str, 
        ...
        domain_priorities = {
            ...
            'infrastructure': ['infrastructure-engineer', 'docker-specialist', 'environment-analyst'],
            'security': ['security-auditor', 'security-enforcer', 'code-quality-specialist'],
    ...
    def _determine_coordination_strategy(self, domain_config: Dict[str, Any], 
    ...
    def _calculate_optimization_confidence(self, domain: str, analysis: Dict[str, Any], 
    ...
    def _default_optimization(self, coordination_request: str, available_agents: List[str]) -> Dict[str, Any]:
        ...
        if domain not in self.performance_baselines:
            ...
            for metric, value in metrics.items():
                if metric in self.performance_baselines[domain]:
                    ...
                    self.performance_baselines[domain][metric] = (1 - alpha) * current + alpha * value
                else:
...
class EnhancedMultiDomainContextReasoner:
    ...
    def _initialize_default_relationships(self):
        ...
        relationships = [
            ...
            DomainRelationship('testing', 'performance', DomainRelationshipType.PARALLEL, 0.8),
            
            # Infrastructure relationships
            DomainRelationship('infrastructure', 'security', DomainRelationshipType.HIERARCHICAL, 0.9),
            DomainRelationship('infrastructure', 'performance', DomainRelationshipType.SYNERGISTIC, 0.8),
            
            # Security relationships
            DomainRelationship('security', 'performance', DomainRelationshipType.CONFLICTING, 0.6),
            
            # Bidirectional relationships
            DomainRelationship('testing', 'infrastructure', DomainRelationshipType.SYNERGISTIC, 0.7, True)
        ]
    ...
    def analyze_multi_domain_query(self, query: str) -> Dict[str, Any]:
        ...
        secondary_domains = [d[0] for d in sorted_domains[1:] if d[1]['confidence'] >= 0.2]
        
        # Analyze domain relationships
        domain_relationships = self._analyze_domain_relationships(primary_domain, secondary_domains)
        
        # Determine pattern complexity
        complexity = self._determine_pattern_complexity(semantic_analysis, domain_relationships)
    ...
    def _analyze_domain_relationships(self, primary_domain: str, 
                                    secondary_domains: List[str]) -> List[Dict[str, Any]]:
    ...
    def _determine_pattern_complexity(self, semantic_analysis: Dict[str, Any], 
                                    domain_relationships: List[Dict[str, Any]]) -> PatternComplexity:
        ...
        avg_path_length = sum(rel['path_length'] for rel in domain_relationships) / max(len(domain_relationships), 1)
    ...
    def _generate_coordination_strategy(self, primary_domain: str, secondary_domains: List[str],
                                      complexity: PatternComplexity, query: str) -> Dict[str, Any]:
        """Generate optimal coordination strategy."""
        # Get domain optimization
        available_agents = self._get_available_agents()  # This would come from agent registry
    ...
    def _select_context_preservation_strategy(self, complexity: PatternComplexity, 
                                            domain_count: int) -> ContextPreservationStrategy:
    ...
    def _estimate_execution_time(self, approach: str, agent_count: int, 
                               complexity: PatternComplexity) -> float:
    ...
    def _calculate_reasoning_confidence(self, semantic_analysis: Dict[str, Any],
        ...
        strategy_confidence = 0.8  # This would be calculated based on actual agent availability
    ...
    def _get_available_agents(self) -> List[str]:
        ...
        return [
            ...
            'infrastructure-engineer', 'security-auditor', 'performance-optimizer',
            'docker-specialist', 'coverage-optimizer', 'security-enforcer'
        ]
...
def example_usage():
    ...
    for i, query in enumerate(test_queries, 1):
        ...
        print(f"Selected Agents: {analysis['coordination_strategy']['selected_agents']}")
