    ...
    def check_agent_file(self, file_path: Path) -> Dict:
        ...
        for i, (subagent_type, description, prompt) in enumerate(task_blocks):
            ...
            if not subagent_type.strip():
                ...
                warnings.append(f"Task {block_num}: Subagent type '{subagent_type}' should use lowercase with hyphens")
                
            # Validate description
            if not description.strip():
                errors.append(f"Task {block_num}: Empty description")
            elif len(description.strip()) < 10:
                warnings.append(f"Task {block_num}: Description too short ({len(description)} chars)")
                
            # Validate prompt
            if not prompt.strip():
                errors.append(f"Task {block_num}: Empty prompt")
            elif len(prompt.strip()) < 20:
                warnings.append(f"Task {block_num}: Prompt too short ({len(prompt)} chars)")
