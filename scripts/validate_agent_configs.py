...
class AgentConfigValidator:
    ...
    def validate_yaml_frontmatter(self, filename: str, content: str) -> None:
        ...
        for field, pattern in required_fields.items():
            ...
            if match:
                ...
                self.errors.append(f"{filename}: Missing required field '{field}' in YAML")
                
        # Validate name matches filename
        if 'name' in yaml_data:
            expected_name = filename.replace('.md', '')
            if yaml_data['name'] != expected_name:
                self.errors.append(f"{filename}: Name '{yaml_data['name']}' doesn't match filename '{expected_name}'")
    ...
    def validate_task_patterns(self, filename: str, content: str) -> None:
        ...
        if task_count == 0:
            self.warnings.append(f"{filename}: No Task() patterns found (expected for primary agents)")
        elif task_count < 5:
            self.warnings.append(f"{filename}: Only {task_count} Task() patterns found (recommended: 5+)")
        else:
            print(f"  ✅ {task_count} Task() patterns found")
            
        # Validate Task() pattern structure
        task_blocks = re.findall(r'Task\(\s*subagent_type="([^"]+)"\s*,\s*description="([^"]+)"\s*,\s*prompt="([^"]+)"\s*\)', content, re.DOTALL)
        
        for i, (subagent_type, description, prompt) in enumerate(task_blocks):
            if not subagent_type.strip():
                self.errors.append(f"{filename}: Task() pattern {i+1} missing subagent_type")
            if not description.strip() or len(description.strip()) < 10:
                self.errors.append(f"{filename}: Task() pattern {i+1} description too short")
    ...
    def validate_content_structure(self, filename: str, content: str) -> None:
        ...
        for section_pattern in required_sections:
            if not re.search(section_pattern, content):
                self.warnings.append(f"{filename}: Missing recommended section matching '{section_pattern}'")
                
        # Check file length (recommended max 500 lines)
        line_count = len(content.split('\n'))
        if line_count > 500:
            self.warnings.append(f"{filename}: File has {line_count} lines (recommended max: 500)")
        elif line_count < 50:
            self.warnings.append(f"{filename}: File has only {line_count} lines (seems too short)")
        else:
