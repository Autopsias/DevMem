...
def validate_agents_section(agents_config: Dict[str, Any]) -> List[str]:
    ...
    if "framework_config" in agents_config:
        ...
        for flag in boolean_flags:
            if flag in framework_config and not isinstance(
                framework_config[flag], bool
            ):
        ...
        for target, min_val, max_val in numeric_targets:
            if target in performance_targets:
                try:
                    ...
                    if not (min_val <= value <= max_val):
                        errors.append(
                            f"agents.performance_targets.{target} must be between {min_val} and {max_val}"
                        )
                except (ValueError, TypeError):
                    errors.append(
                        f"agents.performance_targets.{target} must be a valid number"
                    )
...
def _print_validation_success(config: Dict[str, Any]) -> None:
    ...
    print(
        f"  • Permission rules: {len(config.get('permissions', {}).get('allow', []))} allowed"
    )
    print(f"  • Hook configurations: {len(config.get('hooks', {}))}")
    print(
        f"  • Agent framework version: {config.get('agents', {}).get('version', 'N/A')}"
    )
