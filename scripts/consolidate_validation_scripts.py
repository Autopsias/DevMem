...
def find_validation_scripts() -> List[Path]:
    ...
    subdirs_to_check = [project_root / "scripts", project_root / "tests"]
...
def run_integrated_framework() -> Tuple[bool, str]:
    ...
    try:
        ...
        # Capture summary
        passed_suites = sum(
            1 for suite in framework.test_results if suite.overall_passed
        )
...
def compare_results(
    old_results: Dict[str, Tuple[bool, str]], new_result: Tuple[bool, str]
) -> None:
    ...
    print(
        f"   Overall: {old_passed}/{old_total} scripts passed ({(old_passed/old_total):.1%})"
    )
...
def create_backup_archive() -> Path:
    ...
    with open(manifest_path, "w") as f:
        ...
        f.write(
            "**Purpose:** Backup of individual validation scripts before consolidation\n\n"
        )
        ...
        f.write("\n## Migration Notes\n\n")
        f.write(
            "These scripts were consolidated into the integrated validation framework:\n"
        )
        f.write("`tests/test_integrated_validation_framework.py`\n\n")
        f.write(
            "To run consolidated validations: `pytest tests/test_integrated_validation_framework.py`\n"
        )
...
def create_migration_documentation() -> None:
    ...
    with open(docs_path, "w") as f:
        ...
        f.write("## Overview\n\n")
        f.write(
            "This document describes the migration from individual validation scripts "
        )
        ...
        f.write(
            "- `tests/test_integrated_validation_framework.py` - All validation logic\n"
        )
        ...
        f.write(
            "pytest tests/test_integrated_validation_framework.py::TestIntegratedValidationFramework -v\n"
        )
        f.write("```\n\n")
        ...
        f.write(
            "python tests/test_integrated_validation_framework.py --suite agent-selection\n\n"
        )
        f.write(
            "# Available suites: story, s63, agent-selection, infrastructure, config, learning\n"
        )
        f.write("```\n\n")

        f.write("## Benefits\n\n")
        f.write(
            "- **Reduced Duplication:** Single framework instead of multiple scripts\n"
        )
        f.write(
            "- **Better Maintainability:** Unified codebase for all validation logic\n"
        )
        f.write(
            "- **Improved Reporting:** Comprehensive validation reports with statistics\n"
        )
