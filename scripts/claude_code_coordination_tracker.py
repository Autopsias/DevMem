...
class ClaudeCodeCoordinationTracker:
    ...
    def complete_coordination(
        self, event_id: str, success: bool = True, error_message: Optional[str] = None
    ) -> None:
        """
        ...
        for event in reversed(self.events):
            if (
                ...
                and event.event_type == CoordinationEventType.START
            ):
                start_event = event
                break
        ...
        completion_event = CoordinationEvent(
            event_id=event_id,
            event_type=(
                CoordinationEventType.COMPLETE
                if success
                else CoordinationEventType.ERROR
            ),
            timestamp=time.time(),
            ...
            success=success,
            agents_used=(
                start_event.agents_used.copy()
                if start_event and start_event.agents_used
                else None
            ),
    ...
    def _learn_pattern(
        self, start_event: CoordinationEvent, completion_event: CoordinationEvent
    ) -> None:
        """
        ...
        pattern_key = self._generate_pattern_key(
            start_event.domains, start_event.agent_count, start_event.strategy
        )

        # Determine pattern type
        pattern_type = self._classify_pattern_type(
            start_event.strategy, start_event.agent_count
        )
        ...
        if pattern_key in self.patterns:
            ...
            pattern.avg_duration = (
                total_duration + completion_event.duration
            ) / pattern.usage_count
            ...
            pattern.confidence_score = min(
                0.95, pattern.usage_count * 0.1 + pattern.success_rate * 0.5
            )
    ...
    def _generate_pattern_key(
        self, domains: List[str], agent_count: int, strategy: str
    ) -> str:
    ...
    def _generate_analytics(self) -> Dict[str, Any]:
        ...
        completed_events = [
            ...
            if e.event_type
            in [CoordinationEventType.COMPLETE, CoordinationEventType.ERROR]
        ]
        ...
        pattern_stats.sort(
            key=lambda x: (x["confidence_score"], x["usage_count"]), reverse=True
        )
        ...
        analytics = {
            "summary": {
                ...
                "success_rate": (
                    round(len(successful_events) / len(completed_events), 3)
                    if completed_events
                    else 0
                ),
            ...
            "domain_analytics": {
                domain: {
                    ...
                    "success_rate": (
                        round(domain_success[domain] / domain_usage[domain], 3)
                        if domain_usage[domain] > 0
                        else 0
                    ),
                }
            ...
            "strategy_analytics": {
                strategy: {
                    ...
                    "success_rate": (
                        round(strategy_success[strategy] / strategy_usage[strategy], 3)
                        if strategy_usage[strategy] > 0
                        else 0
                    ),
                    "avg_duration": (
                        round(
                            sum(strategy_duration[strategy])
                            / len(strategy_duration[strategy]),
                            2,
                        )
                        if strategy_duration[strategy]
                        else 0
                    ),
                }
    ...
    def generate_insights(self) -> List[PerformanceInsight]:
        ...
        if analytics and not analytics.get("message"):
            ...
            high_performers = [
                ...
                if p.success_rate > 0.9 and p.usage_count >= 2
            ]
            if high_performers:
                best_pattern = max(
                    high_performers, key=lambda x: x.success_rate * x.confidence_score
                )
                insight = PerformanceInsight(
            ...
            recent_patterns = [
                p for p in self.patterns.values() if p.last_used > current_time - 3600
            ]
            if recent_patterns:
                avg_recent_success = sum(p.success_rate for p in recent_patterns) / len(
                    recent_patterns
                )
            ...
            if total_coordinations > 10:
                for strategy, stats in strategy_stats.items():
                    if (
                        ...
                        and stats["usage_count"] / total_coordinations < 0.1
                    ):
                        insight = PerformanceInsight(
        ...
        self.insights = [
            i for i in self.insights if i.created_at > current_time - 86400
        ]
    ...
    def get_pattern_recommendations(
        self, domains: List[str], agent_count: int
    ) -> Dict[str, Any]:
        """
        ...
        for pattern in self.patterns.values():
            ...
            if domain_overlap and abs(pattern.agent_count - agent_count) <= 2:
                similarity_score = len(domain_overlap) / max(
                    len(domains), len(pattern.domains)
                )
...
def main():
    ...
    tracker.start_coordination(
        event_id, 4, ["infrastructure", "security"], "batch_parallel"
    )
    ...
    tracker.start_coordination(
        event_id, 6, ["testing", "performance"], "batch_parallel"
    )
    time.sleep(0.05)  # Simulate quick failure
    tracker.complete_coordination(
        event_id, success=False, error_message="Timeout occurred"
    )
