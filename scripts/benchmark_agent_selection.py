...
class AgentSelectionBenchmark:
    ...
    def _create_comprehensive_test_set(self) -> List[TestQuery]:
        ...
        return [
            ...
            TestQuery(
                "unit tests not passing coverage requirements",
                "test-specialist",
                (0.6, 0.9),
            ),
            TestQuery(
                "integration test fixtures need improvement",
                "test-specialist",
                (0.6, 0.9),
            ),
            TestQuery(
                "async test patterns causing race conditions",
                "test-specialist",
                (0.7, 1.0),
            ),
            TestQuery(
                "mock configuration breaking test assertions",
                "test-specialist",
                (0.7, 1.0),
            ),
            ...
            TestQuery(
                "kubernetes cluster service mesh problems",
                "infrastructure-engineer",
                (0.7, 1.0),
            ),
            TestQuery(
            ...
            TestQuery(
                "deployment pipeline automation failing",
                "infrastructure-engineer",
                (0.6, 0.9),
            ),
            TestQuery(
            ...
            TestQuery(
                "authentication flow security audit needed",
                "security-enforcer",
                (0.7, 1.0),
            ),
            TestQuery(
                "compliance validation for data encryption",
                "security-enforcer",
                (0.7, 1.0),
            ),
            TestQuery(
                "OAuth2 token security hardening required",
                "security-enforcer",
                (0.7, 1.0),
            ),
            TestQuery(
                "RBAC policy enforcement and validation",
                "security-enforcer",
                (0.6, 0.9),
            ),
            ...
            TestQuery(
                "CPU utilization analysis and tuning",
                "performance-optimizer",
                (0.7, 1.0),
            ),
            TestQuery(
            ...
            TestQuery(
                "throughput optimization for high load",
                "performance-optimizer",
                (0.6, 0.9),
            ),
            # Intelligent-enhancer queries
            TestQuery(
                "refactor code with better variable naming",
                "intelligent-enhancer",
                (0.6, 1.0),
            ),
            TestQuery(
                "function splitting and architecture improvement",
                "intelligent-enhancer",
                (0.7, 1.0),
            ),
            TestQuery(
                "type annotation system enhancement needed",
                "intelligent-enhancer",
                (0.6, 0.9),
            ),
            TestQuery(
            ...
            TestQuery(
                "architectural pattern modernization",
                "intelligent-enhancer",
                (0.5, 0.8),
            ),
            ...
            TestQuery(
                "GitHub Actions workflow optimization", "ci-specialist", (0.6, 0.9)
            ),
            TestQuery(
                "CI pipeline automation and configuration", "ci-specialist", (0.7, 1.0)
            ),
            TestQuery(
                "build automation and deployment pipeline", "ci-specialist", (0.6, 0.9)
            ),
            # Edge cases and ambiguous queries
            TestQuery(
                "something is broken with the tests", "test-specialist", (0.4, 0.7)
            ),
            TestQuery(
                "performance and security issues", "performance-optimizer", (0.4, 0.7)
            ),
            TestQuery(
                "help with docker and testing", "infrastructure-engineer", (0.3, 0.6)
            ),
            TestQuery(
                "need to improve code quality", "intelligent-enhancer", (0.4, 0.7)
            ),
    ...
    def run_load_test(self, matcher, num_queries: int = 1000) -> Dict[str, float]:
        ...
        queries = [tq.query for tq in self.test_queries] * (
            num_queries // len(self.test_queries) + 1
        )
    ...
    def run_comprehensive_benchmark(self) -> Dict[str, BenchmarkResult]:
        ...
        enhanced_result = self.benchmark_matcher(
            self.enhanced_selector, "Enhanced Selector"
        )
    ...
    def print_benchmark_results(self, results: Dict[str, BenchmarkResult]):
        ...
        for name, result in results.items():
            print(f"\n{result.name}:")
            print(
                f"  Accuracy:          {result.accuracy:.2%} ({result.correct_selections}/{result.total_queries})"
            )
        ...
        if "current" in results and "enhanced" in results:
            ...
            accuracy_improvement = (
                (enhanced.accuracy - current.accuracy) / current.accuracy * 100
            )
            time_change = (
                (enhanced.avg_processing_time_ms - current.avg_processing_time_ms)
                / current.avg_processing_time_ms
                * 100
            )
            confidence_improvement = (
                (enhanced.avg_confidence - current.avg_confidence)
    ...
    def run_load_test_comparison(self):
        ...
        throughput_improvement = (
            (enhanced_load["queries_per_second"] - current_load["queries_per_second"])
            / current_load["queries_per_second"]
            * 100
        )
